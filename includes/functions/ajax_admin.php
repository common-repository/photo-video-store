...
function pvs_ajax_recognition_imagga()
{
	...
	$url = 'https://api.imagga.com/v1/tagging?url=' . $_REQUEST["url"] . '&language=' . $_REQUEST["lang"];
	
	$auth = base64_encode( $pvs_global_settings["imagga_key"] . ':' . $pvs_global_settings["imagga_password"] );
	...
	if ( ! is_wp_error( $result ) ) {
		...
		if ( isset( $json_response->results[0]->tags ) )
		{
			foreach ( $json_response->results[0]->tags as $key => $value )
			{
				if ( $tags_result != '' )
				{
					$tags_result .= ',';
...
function pvs_ajax_amazon_test()
{
	...
	if ( wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-amazon-test' ) ) {
...
//Package status
add_action( 'wp_ajax_pvs_package_status',
	'pvs_ajax_package_status' );

function pvs_ajax_package_status()
{
	global $_REQUEST;
	global $rs;
	global $dr;
	...
	$id = ( int )@$_REQUEST['id'];
	$sql = "select id,active from " . PVS_DB_PREFIX . "packages where id=" . $id;
	$rs->open( $sql );
	if ( ! $rs->eof ) {
		if ( $rs->row["active"] == 1 ) {
			$sql = "update " . PVS_DB_PREFIX . "packages set active=0 where id=" . $id;
			$db->execute( $sql );
			?>
				<a href="javascript:pvs_package_status(<?php echo $id?>);"><span class="label label-danger"><?php echo pvs_word_lang( "pending" )?></span></a>
			<?php
		} else {
				$sql = "update " . PVS_DB_PREFIX . "packages set active=1 where id=" . $id;
				$db->execute( $sql );
		?>
				<a href="javascript:pvs_package_status(<?php echo $id?>);"><span class="label label-success"><?php echo pvs_word_lang( "active" )?></span></a>
			<?php
		}
	}
}
...
function pvs_ajax_deselect_row()
{
	...
	$sql = "update " . PVS_DB_PREFIX . "administrators_stats set property_value_old=" .
						(int)@$_REQUEST["id"] . " where administrator_id=" . get_current_user_id() . " and property='" .
						pvs_result( @$_REQUEST["property"] ) . "' and property_value_old < " . (int)@$_REQUEST["id"];
	$db->execute( $sql );
}
...
function pvs_ajax_coupon_status()
{
	...
	if ( wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-coupons-status' ) ) {
		$id = ( int )@$_REQUEST['id'];
...
function pvs_ajax_gateway_status()
{
	...
	if ( wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-gateways' ) ) {
		$i = 1;
...
function pvs_ajax_order_status()
{
	...
	if ( wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-order-status' ) ) {
...
//Check remote file 
add_action( 'wp_ajax_pvs_check_remote',
	'pvs_ajax_check_remote' );

function pvs_ajax_check_remote()
{
	global $_REQUEST;

	$remote = pvs_result($_REQUEST['remote']);

	if ( ! pvs_remote_check( $remote ) ) {
		echo ( 0 );
	} else {
		echo ( 1 );
	}
...
//Dropzone uploader list
add_action( 'wp_ajax_pvs_dropzone_list', 'pvs_ajax_dropzone_list' );

function pvs_ajax_dropzone_list()
{
	global $_REQUEST;
	
	if ( wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-dropzone-list' ) ) {
		echo('<table class="tabel table-striped" style="width:100%;margin:0 20px">');
	
		$dir = opendir( pvs_upload_dir() . "/content/user_" . get_current_user_id() );
		while ( $file = readdir( $dir ) ) {
			if ( $file <> "." && $file <> ".." && preg_match( "/\.(jpg|jpeg)$/i", $file ) ) {
				...
				$size = getimagesize( pvs_upload_dir() . "/content/user_" . get_current_user_id() . "/" . $file, $info );
				if ( isset( $info["APP13"] ) ) {
				...
				$exif_info = @exif_read_data( pvs_upload_dir() . "/content/user_" . get_current_user_id() . "/" . $file, 0, true );
				
				if ( isset( $exif_info["IFD0"]["Title"] ) and $exif_info["IFD0"]["Title"] != ''  ) {
					$title = preg_replace("/[^a-z0-9 ,;\.\-]*/iU","",$exif_info["IFD0"]["Title"]);
				}	
				
				if ( isset( $exif_info["IFD0"]["ImageDescription"] ) and $exif_info["IFD0"]["ImageDescription"] != ''  ) {
					$description = preg_replace("/[^a-z0-9 ,;\.\-]*/iU","",$exif_info["IFD0"]["ImageDescription"]);
				}
				
				if ( isset( $exif_info["IFD0"]["Keywords"] ) and $exif_info["IFD0"]["Keywords"] != ''  ) {
					$keywords = preg_replace("/[^a-z0-9 ,;\.\-]*/iU","",$exif_info["IFD0"]["Keywords"]);
				}
			
				$key = substr(md5($file),0,7);
				
				echo ('<tr valign="top">
							<td><img src="' . site_url() . '/admin_photo_preview2/?file=' . $file .  '" class="img-thumbnail" style="max-width:120px;max-height:120px;margin:10px" /></td>
							<td style="width:60%">
								<input type="text" class="form-control" name="title_' . $key . '" placeholder="' . pvs_word_lang('title') . '" style="margin:5px 0;display:block" value="' . $title . '">
								<textarea class="form-control" name="description_' . $key . '" placeholder="' . pvs_word_lang('description') . '" style="margin:5px 0;display:block">' . $description . '</textarea>
								<textarea class="form-control" name="keywords_' . $key . '" placeholder="' . pvs_word_lang('keywords') . '" style="margin:5px 0;display:block">' . $keywords . '</textarea>
							</td>
							<td><button type="button" class="btn btn-sm btn-link remove_image" id="'. $file .'"><i class="fa fa-trash"></i> ' . pvs_word_lang('Delete') . '</button></td>
						</tr>');
...
//Dropzone uploader. Delete uploaded images
add_action( 'wp_ajax_pvs_dropzone_delete',
	'pvs_ajax_dropzone_delete' );

function pvs_ajax_dropzone_delete()
{
	global $_REQUEST;
	
	if ( wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-dropzone-delete' ) ) {
		$upload_dir = pvs_upload_dir() . "/content/user_" . get_current_user_id() . "/" ;
		
		if ( pvs_result_file( $_REQUEST["name"] ) != '') {
			@unlink( $upload_dir  . pvs_result_file( $_REQUEST["name"] ) );
		}
	}
}
?>