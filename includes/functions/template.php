...
//Get PVS content for theme
function pvs_content() {
	global $title;
	global $sep;	
	global $seplocation;	
	...
	//WP title
	add_filter( 'pre_get_document_title', 'pvs_wp_title' );
	
	//WP head
	add_filter( 'wp_head', 'pvs_wp_head' );
	...
	//Box photographers
	pvs_box_photographers();
}

//WP title
function pvs_wp_title(){
	$title = pvs_get_social_meta_tags( 'pagename' );
	if ( $title != '') {
...
//WP head
function pvs_wp_head(){
	echo(pvs_get_social_meta_tags( 'meta_tags' ));
}
...
function pvs_template_include( $template ) {
	if(get_query_var('pvs_page') != '' and file_exists(get_stylesheet_directory() . '/pvs.php')){
		...
		if(get_query_var('pvs_page') != '' and file_exists( PVS_PATH . 'templates/pvs.php')){
				return PVS_PATH . 'templates/pvs.php';
		}
	}
...
function pvs_is_home_page () {
	...
	//if ( get_query_var('pvs_page') != 'category' and get_query_var('pvs_page') != 'lightbox' and get_query_var('pvs_page') != 'collection' and count( $_POST ) == 0 and ( count( $_GET ) == 0 or ( isset( $_GET["aff"] ) and count( $_GET ) == 1 )) or isset( $_GET["customize_theme"] )  or ( isset( $_GET["bypass"] ) and count($_GET) == 1 ) ) {
	if ( get_query_var('pvs_page') == '' and count( $_POST ) == 0 and ( count( $_GET ) == 0 or ( isset( $_GET["aff"] ) and count( $_GET ) == 1 )) or isset( $_GET["customize_theme"] )  or ( isset( $_GET["bypass"] ) and count($_GET) == 1 ) ) {
		$home_flag = true;
	}
...
function pvs_box_languages () {
	...
	foreach ( $pvs_site_langs as $key => $value ) {
		...
		$box_languages_list_content .= "<li><a href='" . site_url() .
			"/language-select/?lang=" . $key . "'><img src='" . pvs_plugins_url() .
			"/assets/images/languages/" . $lng3 . ".gif' " . $lt . ">" . $key . "</a></li>";
	}
	
	$box_languages_list = "<ul>" . $box_languages_list_content . "</ul>";
	...
	$pvs_theme_content[ 'languages_list_content' ] = $box_languages_list_content;
}
...
function pvs_box_stats () {
	...
	if ( @$pvs_global_settings["allow_photo"] == 1 ) {
		...
		if ( ! $rs->eof ) {
			...
			$vt += (int)$rs->row["count_viewed"];
			$downloads += (int)$rs->row["count_downloaded"];
