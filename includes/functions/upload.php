...
/***************************************************************************
*                                                                         																*
*   Copyright (c) 2006-2018 CMSaccount Inc. All rights reserved.     	   									*
*                                                                         																*
...
function pvs_build_menu_admin_categories( $t_id, $otstup )
{
	...
	while ( ! $dp->eof )
	{
		...
		<td><div class='link_delete'><a href='" . pvs_plugins_admin_url( 'categories/index.php' ) .
				"&action=delete&id=" . $dp->row["id"] . "&_wpnonce=" . wp_create_nonce('pvs-categories-delete' . $dp->row["id"]) . "' onClick=\"return confirm('" .
...
function pvs_rights_managed_upload_form( $type, $rights_managed, $id, $admin_session )
{
	...
	$res = '<script>
	function change_rights_upload_url(value)
	{
		if (value == 1) 
		{
			jQuery(".rights_upload_area").css("display","none");
			jQuery(".rights_url_area").css("display","block");
		}
		else
		{
			jQuery(".rights_upload_area").css("display","block");
			jQuery(".rights_url_area").css("display","none");
		}
	}	
	
	function check_remote_rights(form_field_id, form_field_class, file_format, text_area_id)
	{
		value = jQuery("#"+form_field_id).val();
		jQuery("."+form_field_class).not("#"+form_field_id).val(value);
		...
		if ( value != "")
		{
			jQuery.ajax({
				type:"POST",
				url:ajaxurl,
				data:"action=pvs_check_remote&remote=" + value,
				success:function(data){
					...
					if (data == 1)
					{
						regexp = new RegExp(file_format, "i");
						result = value.match( regexp );
						
						if( result )
						{
							result2 = value.match( /^http/i );
							if( result2 )
							{
								jQuery("."+text_area_id).html("");
							}
							else
							{
								jQuery("."+text_area_id).html("<span style=\"color:red\">' . pvs_word_lang('URL must start with http:// or https://') . '</span>");
							}
						}
						else
						{
							jQuery("."+text_area_id).html("<span style=\"color:red\">' . pvs_word_lang('Incorrect file type') . '</span>");
						}
					}
					else
					{
						jQuery("."+text_area_id).html("<span style=\"color:red\">' . pvs_word_lang('unable to open remote file') . '</span>");
					}
				}
			});
		}
	...
	$flag_remote_storage = false;
	$remote_files = array();

	if ( pvs_is_remote_storage() and $id != 0 ) {
		$sql = "select url,filename1,filename2,item_id from " . PVS_DB_PREFIX . "filestorage_files where item_id<>0 and id_parent=" . ( int )$id;
		$dn->open( $sql );
		while ( ! $dn->eof ) {
			$flag_remote_storage = true;
			$remote_files[ $id ] = $dn->row[ 'url' ] . '/' . $dn->row[ 'filename2' ];
			$dn->movenext();
		}
	}
	...
	if ( ! $ds->eof )
	{
		...
		if ( $file_form )
		{
			...
			if ( pvs_is_user_admin () ) {
				...
				$res .= "<th colspan='2'><input type='radio' name='rights_url_form' id='rights_url1' value='0' " . $check1 . " onClick='change_rights_upload_url(0);'>&nbsp;<label for='rights_url1'>" . pvs_word_lang("upload") . "</label>&nbsp;&nbsp;&nbsp;&nbsp;<input type='radio' name='rights_url_form' id='rights_url2' " . $check2 . " value='1'  onClick='change_rights_upload_url(1);'>&nbsp;<label for='rights_url2'>" . pvs_word_lang("URL") . "</label></th>";
			} else {
				$res .= "<th colspan='2'><b>" . pvs_word_lang( "file" ) . ":</b></th>";
			}
			...
			if ( ( $type == "photo" and $pvs_global_settings["photo_uploader"] ==
				...
				"usual uploader" ) )
			{
				$res .= "<div class='rights_upload_area'><input name='video_rights' type='file' style='width:400px' class='ibox form-control'></div>";
			}
			...
			if ( ( $type == "photo" and $pvs_global_settings["photo_uploader"] ==
				...
				"jquery uploader" ) )
			{
				$res .= "<div class='rights_upload_area'><span class=\"btn btn-success fileinput-button\">
					...
					pvs_word_lang( "select files" ) . "...
    					...
    					<div id=\"files100\" class=\"files\"></div></div>
    					";

			}
			
			if ( pvs_is_user_admin () ) {
				$res .= "<div class='rights_url_area' style='display:none'><input class='ibox form-control rights-url-field'  id='rights_url'  name='rights_url' type='text' style='width:100%'  value='" . @ $remote_files[ $id ] . "'  onKeyup='check_remote_rights(\"rights_url\", \"rights-url-field\",\"(" . $filetypes2 . ")$\", \"rights_area\")'><div class='rights_area'></div></div>";
			}
					...
					} else
					{
						if ( $type != "photo" )
						{
							$res .= "<tr><td colspan='2'><a href='" . $remote_file .
								"' class='btn btn-default'><i class='icon-download'></i> " . pvs_word_lang( "download" ) .
		...
		if ( pvs_is_user_admin () and $flag_remote_storage ) {
			$res .= "<script>change_rights_upload_url(1);</script>";
		}

		$res .= pvs_get_preview_form( $type, true, $id );
...
function pvs_create_temp_folder( $remove = true )
{
...
function pvs_photo_upload_form( $id, $admin_session )
{
	...
	$res = '<script>
	function change_file_upload_url(value)
	{
		if (value == 1) 
		{
			jQuery(".file_upload_area").css("display","none");
			jQuery(".file_url_area").css("display","block");
		}
		else
		{
			jQuery(".file_upload_area").css("display","block");
			jQuery(".file_url_area").css("display","none");
		}
	}	
	
	function check_remote(form_field_id, form_field_class, file_format, text_area_id)
	{
		value = jQuery("#"+form_field_id).val();
		jQuery("."+form_field_class).not("#"+form_field_id).val(value);
	...
	$filetypes = "jpe?g|png|gif|raw|tif?f|eps|jp2|jpf|zip";
	...
	if ( pvs_is_remote_storage() and $id != 0 ) {
		...
		while ( ! $dn->eof ) {
			$flag_remote_storage = true;
			$photo_extention = pvs_get_file_info( $dn->row[ 'filename1' ], "extention" );
			$remote_files[ $photo_extention ] = $dn->row[ 'url' ] . '/' . $dn->row[ 'filename2' ];
			$dn->movenext();
		}
	}
	...
	{
		$sql = "select server1,url_jpg,url_png,url_gif,url_raw,url_tiff,url_eps,url_jp2,url_jpf,url_zip from " .
					...
					if ( ! $flag_storage )
					{
						...
						if ( pvs_is_user_admin () )
						{
							$rurl .= "<a href='" . pvs_plugins_admin_url('catalog/index.php') . "&action=photo_delete&id=" . ( int )$id . "&file=" . $value .
								"&_wpnonce=" . wp_create_nonce('pvs-photo-delete') . "' class='btn btn-danger'><i class='icon-remove icon-white'></i> " .
	...
	if ( pvs_is_user_admin () ) {
		...
		$res .= "<br><p><input type='radio' name='file_url' id='file_url1' value='0' " . $check1 . " onClick='change_file_upload_url(0);'>&nbsp;<label for='file_url1'>" . pvs_word_lang("upload") . "</label>&nbsp;&nbsp;&nbsp;&nbsp;<input type='radio' name='file_url' id='file_url2' " . $check2 . " value='1'  onClick='change_file_upload_url(1);'>&nbsp;<label for='file_url2'>" . pvs_word_lang("URL") . "</label></p>";
	}

	$res .= "<table class='profile_table table table-striped table-hover'>";
	if ( $file_form )
	{
		$res .= "<tr class='snd'>
			<td colspan='5'>";
		...
		if ( $pvs_global_settings["photo_uploader"] == "usual uploader" )
		{
			foreach ( $photo_formats as $key => $value )
			{
				...
				$res .= "<div class='file_upload_area'><div style='margin-top:" . $margin . "px'><b>" . strtoupper( $value ) .
					":</b><br><input name='photo_" . strtolower( $value ) .
					"' type='file' style='width:400px' class='ibox form-control'></div></div>";
			}
		}
			...
			foreach ( $photo_formats as $key => $value )
			{
				...
				$res .= "<div class='file_upload_area'><div style='margin-top:" . $margin . "px'><b>" . strtoupper( $value ) .
					...
					$key . "' value=''>
    					...
    					<div id=\"files" . $key . "\" class=\"files\"></div></div></div>
    					";
			}
		}
		...
		foreach ( $photo_formats as $key => $value )
		{
			...
			if ( pvs_is_user_admin () ) {
				$res .= "<div class='file_url_area' style='display:none'><div style='margin-top:" . $margin . "px'><b>" . strtoupper( $value ) .
					":</b><br><input class='ibox form-control file-url-field" . $value . "'  id='file_url" . $value . "'  name='file_url" . $value . "' type='text' style='width:100%'  value='" . @ $remote_files[ $value ] . "'  onKeyup='check_remote(\"file_url" . $value .
						"\", \"file-url-field" . $value . "\",\"(" . str_replace( 'jpg', 'jpe?g', $value ) . ")$\", \"file_area" . $value . "\")'><div class='file_area" . $value . "'></div></div></div>";
			}
		}
	...
	if ( ! $pvs_global_settings["printsonly"] )
	{
		...
		while ( ! $ds->eof )
		{
			...
			$sql = "select id_parent,size,title,price,jpg,png,gif,raw,tiff,eps,jpf,jp2,zip from " .
	...
	if ( pvs_is_user_admin () and $flag_remote_storage ) {
		$res .= "<script>change_file_upload_url(1);</script>";
	}
...
function pvs_files_upload_form( $id, $type, $admin_session )
{
	...
	if ( pvs_is_remote_storage() and $id != 0 ) {
		...
		while ( ! $dn->eof ) {
			$flag_remote_storage = true;
			$remote_files[ $dn->row[ 'item_id' ] ] = $dn->row[ 'url' ] . '/' . $dn->row[ 'filename2' ];
			$dn->movenext();
		}
	}
	...
	if ( pvs_is_user_admin () ) {
		...
		$res .= "<th style='width:50%'><input type='radio' name='file_url' id='file_url1' value='0' " . $check1 . " onClick='change_file_upload_url(0);'>&nbsp;<label for='file_url1'>" . pvs_word_lang("upload") . "</label>&nbsp;&nbsp;&nbsp;&nbsp;<input type='radio' name='file_url' id='file_url2' " . $check2 . " value='1'  onClick='change_file_upload_url(1);'>&nbsp;<label for='file_url2'>" . pvs_word_lang("URL") . "</label></th>";
	} else {
		$res .= "<th style='width:50%'><b>" . pvs_word_lang( "file" ) . ":</b></th>";
	}
	...
	while ( ! $dr->eof )
	{
		...
		while ( ! $ds->eof )
		{
			...
			if ( $id != 0 )
			{
				$sql = "select id,price,url,shipped,name,contacts from " . PVS_DB_PREFIX .
				...
				if ( ! $dd->eof )
				{
					...
					if ( $dd->row["shipped"] != 1 and $dd->row["contacts"] != 1 )
					{
						//Define server
						$server1 = $site_server_activ;
			...
			if ( $ds->row["shipped"] != 1 and $ds->row["contacts"] != 1 )
			{
				$res .= "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;(";
			...
			if ( $ds->row["shipped"] != 1 and $ds->row["contacts"] != 1 )
			{
				if ( ( $type == "video" and $pvs_global_settings["video_uploader"] ==
					...
					"usual uploader" ) )
				{
					$res .= "<div class='file_upload_area'><input name='video_sale" . $ds->row["id_parent"] .
						"' type='file' style='width:200px' class='ibox form-control'></div>";
				}
				...
				if ( ( $type == "video" and $pvs_global_settings["video_uploader"] ==
					...
					"jquery uploader" ) )
				{
					$res .= "<div class='file_upload_area'><span class=\"btn btn-success fileinput-button\">
        					<i class=\"icon-plus icon-white\" style=\"float:left\"></i> " .
						pvs_word_lang( "select files" ) . "...
        					<!-- The file input field used as target for the file upload widget -->
    					...
    					<div id=\"files" . $ds->row["id_parent"] . "\" class=\"files\"></div></div>
    					";
			...
			} else
			{
				...
				if ( $ds->row["contacts"] == 1 )
				{
					$res .= pvs_word_lang( "contact us to get the price" );
				}
			}
			...
			if ( pvs_is_user_admin () ) {
				if ( $ds->row["shipped"] != 1 and $ds->row["contacts"] != 1 )
				{
					$res .= "<div class='file_url_area' style='display:none'><input class='ibox form-control file-url-field" . $ds->row["id_parent"] . "'  id='file_url" . $ds->row["id_parent"] . "'  name='file_url" . $ds->row["id_parent"] . "' type='text' style='width:100%'  value='" . @ $remote_files[ $dd->row[ 'id' ] ] . "'  onKeyup='check_remote(\"file_url" . $ds->row["id_parent"] .
					"\", \"file-url-field" . $ds->row["id_parent"] . "\",\"(" . $url_types . ")$\", \"file_area" . $ds->row["id_parent"] . "\")'><div class='file_area" . $ds->row["id_parent"] . "'></div></div>";
				}
			}
	...
	$res .= pvs_get_preview_form( $type, false, $id );
...
function pvs_price_update( $id, $type )
{
	...
	while ( ! $dr->eof )
	{
		...
		while ( ! $ds->eof )
		{
			$sql = "select price,url,shipped,contacts from " . PVS_DB_PREFIX .
				"items where id_parent=" . ( int )$id . " and price_id=" . $ds->row["id_parent"];
			$dd->open( $sql );
			if ( ! $dd->eof )
			{
				...
				if ( $type == "photo" )
				{
					...
					if ( $photo_file != "" and isset( $_POST[$type . "_chk" . $ds->row["id_parent"]] ) )
					{
						$sql = "insert into " . PVS_DB_PREFIX .
							"items (id_parent,name,url,price,priority,shipped,price_id,contacts) values (" . $id .
							",'" . pvs_result( @$_POST["title" . $ds->row["id_parent"]] ) . "','" . $photo_file .
							"'," . ( float )$_POST[$type . "_price" . $ds->row["id_parent"]] . "," . $ds->
							row["priority"] . ",0," . $ds->row["id_parent"] . "," . (int)$ds->row["contacts"] . ")";
						$db->execute( $sql );
					}
				}
			}
...
function pvs_prints_upload_form()
{
	...
	$res .= "<script>function change_quantity(print,value)
	{
		if(value == -1)
		{
			jQuery('#quantity'+print).css('display','none');
		}
		else
		{
			jQuery('#quantity'+print).css('display','block');
		}
	}
	function checkbox_prints()
	{
		if( jQuery('#prints_all').prop('checked') )
		{
			jQuery('.prints_price').prop('checked', true); 
		}
		else
		{
			jQuery('.prints_price').prop('checked', false); 
		}
	}</script>
	<table border='0' cellpadding='5' cellspacing='1' class='profile_table table table-striped table-hover'>
	<tr>
	<th><input type='checkbox' id='prints_all' checked onClick='checkbox_prints()'></th>
	<th><b>" . pvs_word_lang( "title" ) . ":</b></th>
	<th><b>" . pvs_word_lang( "price" ) . ":</b></th>";
	...
	while ( ! $dr->eof )
	{
		...
		while ( ! $ds->eof )
		{
			...
			<td><input name='prints_chk" . $ds->row["id_parent"] .
				"' type='checkbox' class='prints_price' checked></td>
			<td>" . $ds->row["title"] . "</td>
...
function pvs_prints_live( $id )
{
	...
	while ( ! $dd->eof )
	{
		...
		while ( ! $ds->eof )
		{
			...
			$res .= "<tr " . $row_style . ">
			<td><input name='prints_chk" . $ds->row["id_parent"] . "' type='checkbox' class='prints_price' " .
				$checked . "></td>
			<td>" . $ds->row["title"] . "</td>
			<td><input class='ibox form-control' name='prints_price" . $ds->row["id_parent"] .
...
function pvs_build_admin_form( $url, $type, $wpnonce = 'pvs' )
{
	global $admin_fields;
	...
	global $category_list_package;
	global $items_list_package;
	...
	$form_result .= "<form method='post' Enctype='multipart/form-data' id='uploadform' name='uploadform' action='" .
		...
		$form_result .= wp_nonce_field( $wpnonce );
		...
		if ( $id != 0 and pvs_is_user_admin () ) {
			...
			if ( $type == 'video' ) {	
				...
				if ( ! $dd->eof) {
					if ( isset($_REQUEST["i"])) {
						@copy(pvs_upload_dir() . pvs_server_url( $dd->row["server1"] ) . "/" .
		( int )$id . "/thumb" . ( int )$_REQUEST["i"] . ".jpg", pvs_upload_dir() . pvs_server_url( $dd->row["server1"] ) . "/" .
		( int )$id . "/thumb100.jpg");
					}
			
					$video_previews = array();
					$dir = opendir(  pvs_upload_dir() . pvs_server_url( $dd->row["server1"] ) . "/" . $id );
					while ( $file = readdir( $dir ) )
					{
						if ( $file <> "." && $file <> ".." )
						{
							...
							{
								if ( preg_match( "/.jpg$/i", $file ) and ! preg_match( "/thumb100/i", $file ) )
								{
									$video_previews[] = pvs_upload_dir('baseurl') . pvs_server_url( $dd->row["server1"] ) . "/" . $id . "/" . $file;
								}
							}
						}
					}
					closedir( $dir );
					if( count($video_previews) > 1 ) {
						$form_result .= "<br><h4>" . pvs_word_lang("Select video preview") . ":</h4><table><tr>";
						foreach ( $video_previews as $key => $value ) {
							$form_result .= "<td style='padding:3px'><a href='" . pvs_plugins_admin_url('catalog/index.php') . "&action=content&id=" . $id . "&i=" . $key . "'><img src='" . $value . "' style='max-width:100px'></a></td>";
						}
						$form_result .= "</table></tr>";
					}
				}				
			}
			...
			if ( $pvs_global_settings["clarifai"] or $pvs_global_settings["imagga"] ) {
				$recognition_file = pvs_show_preview( $id, $type, 2, 1 );
				...
				if ( $type == 'video' and $pvs_global_settings["imagga"] ) {
					$recognition_file_imagga = pvs_show_preview( $id, $type, 3, 1 );
				}

				if ( $type == 'audio' ) {
					$recognition_file_clarifai = pvs_show_preview( $id, $type, 3,
						1 );
					$recognition_file_imagga = pvs_show_preview( $id, $type, 3, 1 );
				}
	...
	for ( $i = 0; $i < count( $admin_fields ); $i++ )
	{
			...
			if ( $type == "category")
			{
				...
				if ( $admin_meanings[$i] . "" != "" )
				{
					$form_result .= "<div style='padding-top:3px'><div  style='padding-bottom:3px'><img src='" . pvs_upload_dir( 'baseurl' ) .
						...
						"&_wpnonce=" . wp_create_nonce('pvs-categories-delete-thumb') . "'  class='btn btn-default btn-sm'>" . pvs_word_lang( "delete" ) . "</a></div>";
				}
			}
		...
		if ( $admin_types[$i] == "category_package" ) {
			$form_result .= "<div style='margin-top:20px' id='category_list_package'>" . $category_list_package . "</div>";
		}
		
		if ( $admin_types[$i] == "type_package" ) {
			$form_result .= $items_list_package;
		}
...
//Build input/textarea field with translations
function pvs_get_translation_fields($id,$field,$type)
{
	global $db;
	global $lng;
	global $lang_symbol;
	...
	$results['inputs'] = '';
	$results['lng'] = '';
	$results['lng_translation'] = '';
	
	$sql = "select name,activ from " . PVS_DB_PREFIX . "languages where display=1 order by name";
	$dp->open( $sql );
	while ( ! $dp->eof )
	{
		if ( $dp->row["activ"] == 1 )
		{
			$lng_translation = $dp->row["name"];
		} else {
			$lng3 = strtolower( $dp->row["name"] );
			$lng_symbol = $lang_symbol[$dp->row["name"]];
			if ( $lng3 == "chinese traditional" )
			{
				$lng3 = "chinese";
				$lng_symbol = "zh1";
			...
			$sql = "select title,keywords,description from " . PVS_DB_PREFIX . "translations where id=" . $id . " and lang='" . $lng_symbol . "'";
			$dt->open( $sql );
			if ( ! $dt->eof )
			{
				if ( $dt->row[$field] != "" )
				{
					if ( $type == "input" )
					{
						$input_code = "<input type='text' name='translate_" . $field . "_" .
							$lng_symbol . "' style='' class='ibox form-control' value='" . $dt->
							row[$field] . "'>";
					} else
					{
						$input_code = "<textarea name='translate_" . $field . "_" . $lng_symbol .
							"' style='height:120px' class='ibox form-control'>" . $dt->row[$field] .
							"</textarea>";
					}
	
					$input_list .= "<div class='clearfix' id='div_" . $field . "_" . $lng_symbol .
						"' style='padding-top:20px'><div class='input-append' style='float:left;margin-right:4px;width:80%'>" .
						$input_code . "<span class='add-on' style='width:120px;text-align:left'><img src='" .
						pvs_plugins_url() . "/assets/images/languages/" . $lng3 .
						".gif'>&nbsp;<font class='langtext'>" . $dp->row["name"] .
						"</font></span></div><button class='btn btn-danger' type='button' onClick=\"translation_delete('" .
						$field . "','" . $lng_symbol . "');\">" . pvs_word_lang( "delete" ) .
						"</button></div>";
				} else {
					$lng_list .= "<li id='li_" . $field . "_" . $lng_symbol .
						"' style='float:left;width:170px'><a href=\"javascript:translation_add('" . $field .
						"','" . $dp->row["name"] . "','" . $lng3 . "','" . $lng_symbol . "','" . $type .
						"');\"><img src='" . pvs_plugins_url() . "/assets/images/languages/" . $lng3 .
						".gif'>&nbsp;" . $dp->row["name"] . "</a></li>";
				}
			} else {
				$lng_list .= "<li id='li_" . $field . "_" . $lng_symbol .
					"' style='float:left;width:170px'><a href=\"javascript:translation_add('" . $field .
	...
	$results['languages'] = $lng_list;
	$results['inputs'] = $input_list;
	$results['lng'] = $lng3;
	$results['lng_translation'] = $lng_translation;
...
//Update translations
function pvs_update_translations($id,$type)
{
	global $db;
	global $_POST;
	global $pvs_global_settings;
			...
			if ( $rs->eof )
			{
				...
				if ( $flag )
				{
					$sql = "insert into " . PVS_DB_PREFIX .
						...
						",0," . $dr->row["id_parent"] . "," . (int)$dr->row["contacts"] . ")";
					$db->execute( $sql );
				}
			}
			$dr->movenext();
...
function pvs_publication_watermark_add( $id, $file )
{
	...
	if ( $pvs_global_settings["watermark_photo"] != "" and file_exists( $pvs_global_settings["watermark_photo"] ) )
	{
		pvs_watermark( $file, $pvs_global_settings["watermark_photo"] );
	}
...
function pvs_publication_iptc_add( $id, $photo )
{
	...
	if ( isset( $exif_info["IFD0"]["Title"] ) and $exif_info["IFD0"]["Title"] != ''  ) {
		$title = pvs_result( preg_replace("/[^a-z0-9 ,;\.\-]*/iU","",$exif_info["IFD0"]["Title"]) );
	
		$sql = "update " . PVS_DB_PREFIX . "media set title='" . $title . "' where id=" . $id;
		$db->execute( $sql );
	}	
	
	if ( isset( $exif_info["IFD0"]["ImageDescription"] ) and $exif_info["IFD0"]["ImageDescription"] != ''  ) {
		$description = pvs_result( preg_replace("/[^a-z0-9 ,;\.\-]*/iU","",$exif_info["IFD0"]["ImageDescription"]) );
	
		$sql = "update " . PVS_DB_PREFIX . "media set description='" . $description . "' where id=" . $id;
		$db->execute( $sql );
	}
	
	if ( isset( $exif_info["IFD0"]["Keywords"] ) and $exif_info["IFD0"]["Keywords"] != ''  ) {
		$keywords = pvs_result( preg_replace("/[^a-z0-9 ,;\.\-]*/iU","",$exif_info["IFD0"]["Keywords"]) );
		
		$sql = "update " . PVS_DB_PREFIX . "media set keywords='" . $keywords . "' where id=" . $id;
		$db->execute( $sql );
	}
...
function pvs_publication_files_upload( $id, $type )
{
	...
	if ( $price_license == "royalty_free" )
	{
		...
		while ( ! $ds->eof )
		{
			if ( $ds->row["shipped"] != 1 and $ds->row["contacts"] != 1 )
			{
				...
				//Remote file URLs
				if ( (int)@$_POST[ 'file_url' ] == 1 and pvs_is_user_admin () ) {
					if ( @$_POST[ 'file_url' . $ds->row[ 'id_parent' ] ] != '' and pvs_remote_check( @$_POST[ 'file_url' . $ds->row[ 'id_parent' ] ] )) {
						$url = pvs_result($_POST[ 'file_url' . $ds->row[ 'id_parent' ] ]);
						$url_server = '';
						$url_file = '';
						...
						for ($i = 0; $i < count($url_parts) - 1; $i++) {
							...
							$url_server .= $url_parts[$i]; 
						}
						$url_file = $url_parts[ count($url_parts) - 1 ];
						
						$file_extention = pvs_get_file_info( $url_file, "extention" );
						...
						if ($flag_url) {
							...
							if ( $rs->eof )
							{
								$sql = "insert into " . PVS_DB_PREFIX .
									"items (id_parent,name,url,price,priority,shipped,price_id) values (" . $id .
									",'" . pvs_result( $_POST["title" . $ds->row["id_parent"]] ) . "','" . $url_file .
									"'," . floatval( $_POST[$type . "_price" . $ds->row["id_parent"]] ) . "," . $ds->
									row["priority"] . ",0," . $ds->row["id_parent"] . ")";
								$db->execute( $sql );
								
								$sql = "select id from " . PVS_DB_PREFIX . "items where id_parent=" . $id . " and price_id = " . $ds->row["id_parent"];
								$dp->open( $sql );
								if ( ! $dp->eof ) {
									$sql =" select id_parent from " . PVS_DB_PREFIX . "filestorage_files where id_parent = " . $id . " and item_id = " . $dp->row[ 'id' ];
									$dr->open($sql);
									if ( $dr->eof ) {
										$sql = "insert into " . PVS_DB_PREFIX . "filestorage_files (id_parent,item_id,url,filename1,filename2,filesize,server1,pdelete,width,height) values (" . $id . "," . $dp->row[ 'id' ] . ",'" . $url_server . "','" . $url_file . "','" . $url_file . "'," . $filesize . ",0,0,0,0)";
										$db->execute( $sql );	
									}
								}
							} else
							{
								$sql = "update " . PVS_DB_PREFIX . "items set name='" . pvs_result( $_POST["title" .
									$ds->row["id_parent"]] ) . "',url='" . $url_file . "',price=" . floatval( $_POST[$type .
								...
								if ( ! $dp->eof ) {
									...
									$dr->open($sql);
									if ( ! $dr->eof ) {
										$sql = "update " . PVS_DB_PREFIX . "filestorage_files set url='" . $url_server . "', filename2='" . $url_file . "', filesize=" . $filesize . " where id_parent = " . $id . " and item_id = " . $dp->row[ 'id' ];
										$db->execute( $sql );	
									}
								}
							}
							...
							if ( $ds->row["thesame"] > 0 )
							{
								...
								while ( ! $dp->eof )
								{
									...
									if ( $dt->eof )
									{
										$sql = "insert into " . PVS_DB_PREFIX .
											"items (id_parent,name,url,price,priority,shipped,price_id) values (" . $id .
											",'" . pvs_result( $_POST["title" . $dp->row["id_parent"]] ) . "','" . $url_file .
											"'," . floatval( $_POST[$type . "_price" . $dp->row["id_parent"]] ) . "," . $dp->
											row["priority"] . ",0," . $dp->row["id_parent"] . ")";
										$db->execute( $sql );
										
										$sql = "select id from " . PVS_DB_PREFIX . "items where id_parent=" . $id . " and price_id = " . $dp->row["id_parent"];
										$dq->open( $sql );
										if ( ! $dq->eof ) {
											$sql =" select id_parent from " . PVS_DB_PREFIX . "filestorage_files where id_parent = " . $id . " and item_id = " . $dq->row[ 'id' ];
											$dr->open($sql);
											if ( $dr->eof ) {
												$sql = "insert into " . PVS_DB_PREFIX . "filestorage_files (id_parent,item_id,url,filename1,filename2,filesize,server1,pdelete,width,height) values (" . $id . "," . $dq->row[ 'id' ] . ",'" . $url_server . "','" . $url_file . "','" . $url_file . "'," . $filesize . ",0,0,0,0)";
												$db->execute( $sql );	
											}
										}
									} else
									{
										$sql = "update " . PVS_DB_PREFIX . "items set name='" . pvs_result( $_POST["title" .
											$dp->row["id_parent"]] ) . "',url='" . $url_file . "',price=" . floatval( $_POST[$type .
										...
										if ( ! $dq->eof ) {
											...
											$dr->open($sql);
											if ( ! $dr->eof ) {
												$sql = "update " . PVS_DB_PREFIX . "filestorage_files set url='" . $url_server . "', filename2='" . $url_file . "', filesize=" . $filesize . " where id_parent = " . $id . " and item_id = " . $dq->row[ 'id' ];
												$db->execute( $sql );		
											}
										}
									}
				...
				if ( isset( $_POST[$type . "_chk" . $ds->row["id_parent"]] ) )
				{
					...
					if ( $rs->eof )
					{
						$sql = "insert into " . PVS_DB_PREFIX .
							...
							"," . $ds->row["shipped"] . "," . $ds->row["id_parent"] . "," . $ds->row["contacts"] . ")";
						$db->execute( $sql );
						$swait = true;
					} else
					{
	...
	if ( $price_license == "rights_managed" )
	{
		...
		if ( ! $ds->eof )
		{
			...
			//Remote file URLs
			if ( (int)@$_POST[ 'rights_url_form' ] == 1 and pvs_is_user_admin () ) {
				if ( @$_POST[ 'rights_url' ] != '' and pvs_remote_check( @$_POST[ 'rights_url' ] )) {
					$url = pvs_result($_POST[ 'rights_url' ]);
					$url_server = '';
					$url_file = '';
					...
					if ($flag_url) {
						...
						if ( $rs->eof )
						{
							$sql = "insert into " . PVS_DB_PREFIX .
								"items (id_parent,name,url,price,priority,shipped,price_id) values (" . $id .
								",'" . $ds->row["title"] . "','" . $url_file . "'," . $ds->row["price"] .
								",0,0," . $ds->row["id"] . ")";
							$db->execute( $sql );
							
							$sql = "select id from " . PVS_DB_PREFIX . "items where id_parent=" . $id . " and price_id = " . $ds->row["id"];
							$dp->open( $sql );
							if ( ! $dp->eof ) {
								$sql =" select id_parent from " . PVS_DB_PREFIX . "filestorage_files where id_parent = " . $id . " and item_id = " . $dp->row[ 'id' ];
								$dr->open($sql);
						...
						{
							$sql = "update " . PVS_DB_PREFIX . "items set url='" . $url_file .
	...
	//Remote preview URLs
	if ( ( (int)@$_POST[ 'upload_url' ] == 1 or (int)@$_POST[ 'upload_url_rights' ] == 1 ) and pvs_is_user_admin () ) {
							
		if ( $type != "photo" ) {
		
			if ( isset( $_POST['preview_url'] ) or isset( $_POST['preview_url2'] ) or isset( $_POST['preview_url_rights'] ) or isset( $_POST['preview_url2_rights'] ) ) {
				$sql ="delete from " . PVS_DB_PREFIX . "filestorage_files where id_parent=" . $id . " and item_id=0";
				$ds->open($sql);
			}
		
			if ( $pvs_global_settings[ 'royalty_free' ]) {
				$array_previews = array('preview_url', 'preview_url2');
			} else {
				$array_previews = array('preview_url_rights', 'preview_url2_rights');
			}
			
			foreach ( $array_previews as $key => $value ) {
				if ( @$_POST[ $value ] != '' and pvs_remote_check( $_POST[ $value ] )) {
					$url = pvs_result($_POST[ $value ]);
					$url_server = '';
					$url_file = '';
					$url_file2 = '';
									
					if ( preg_match($pvs_regular_expressions[ 'youtube' ], @$_POST[ 'preview_url' ] ) or preg_match($pvs_regular_expressions[ 'youtube' ], @$_POST[ 'preview_url_rights' ] )  ) {
						$url_server = $url;
						if ( $value == 'preview_url' or $value == 'preview_url_rights') {
							$url_file = 'youtube';
						} else {
							$url_file = 'youtube_photo';
						}
						$url_file2 = '';
					} else if ( preg_match($pvs_regular_expressions[ 'vimeo' ], @$_POST[ 'preview_url' ] ) or  preg_match($pvs_regular_expressions[ 'vimeo' ], @$_POST[ 'preview_url_rights' ] ) ) {
						$url_server = $url;
						if ( $value == 'preview_url' or $value == 'preview_url_rights') {
							$url_file = 'vimeo';
						} else {
							$url_file = 'vimeo_photo';
						}
						$url_file2 = '';
					} else {
						$url_parts = explode("/", $url );
						...
						if ( $value == 'preview_url' or $value == 'preview_url_rights') {
							...
							if ( $type == 'vector') {
								...
								$url_file = 'thumb100.jpg';
							}
						}
						$url_file2 = $url_parts[ count($url_parts) - 1 ];
					...
					if ( $url_file == 'youtube_photo' or $url_file == 'vimeo_photo' ) {
						$size = @getimagesize( $url_server );
						$width = (int)@$size[0];
						$height = (int)@$size[1];
					} else {						
						if ( preg_match("/jpg$/i", $url_file) ) {
							$size = @getimagesize( $url_server . '/' . $url_file2);
							$width = (int)@$size[0];
							$height = (int)@$size[1];
						}
					}
					...
					$sql = "insert into " . PVS_DB_PREFIX .
							...
							$id . ",0,'" . $url_server . "','" . $url_file . "','" . $url_file2 . "',0,0,0," . $width . ", " . $height . ")";
					$db->execute( $sql );			
				}
			}
		}
...
function pvs_publication_photo_upload( $id )
{
	...
	if ( (int)@$_POST[ 'file_url' ] == 1 and pvs_is_user_admin () ) {
		...
		if ( $price_license == "royalty_free" ) {
			foreach ( $photo_formats as $key => $value ) {			
				if ( @$_POST[ 'file_url' . $value ] != '' and pvs_remote_check( @$_POST[ 'file_url' . $value ] )) {
					$url = pvs_result($_POST[ 'file_url' . $value ]);
					$url_server = '';
					$url_file = '';
					...
					$photo_extention = pvs_get_file_info( $url_file, "extention" );
					
					$flag_url = false;
					
					if ( preg_match( "/\.jpg$|\.jpeg$|\.gif$|\.png$|\.raw$|\.eps$|\.tif$|\.tiff$|\.jpf$|\.jp2$|\.zip$/i",
					$url_file ) )
					{
						$flag_url = true;
					}
					...
					$width = 0;
					$height = 0;
					if ( preg_match( "/\.jpg$/i", $url_file ) or preg_match( "/\.jpeg$/i", $url_file ) or
						preg_match( "/\.png$/i", $url_file ) or preg_match( "/\.gif$/i", $url_file ) )
					{
					...
					if ($flag_url) {
						$sql = "select id_parent,title,price,priority from " . PVS_DB_PREFIX . "sizes where " . $value . "=1 order by priority";
						$dp->open( $sql );
						while ( ! $dp->eof ) {
							$sql = "select id,id_parent,url,name,price from " . PVS_DB_PREFIX .
								"items where id_parent=" . $id . " and price_id=" . $dp->row["id_parent"];
							...
							if ( $dt->eof )
							{
								...
								if ( isset( $_POST["photo_chk" . $dp->row["id_parent"]] ) )
								{
									$flag_item = true;
								}
				
								$price = $dp->row["price"];
								if ( isset( $_POST["photo_price" . $dp->row["id_parent"]] ) )
								{
									$price = ( float )$_POST["photo_price" . $dp->row["id_parent"]];
								}
				
								$title = $dp->row["title"];
								if ( isset( $_POST["title" . $dp->row["id_parent"]] ) )
								{
									$title = pvs_result( $_POST["title" . $dp->row["id_parent"]] );
								}
								...
								if ( $flag_item )
								{
									$sql = "insert into " . PVS_DB_PREFIX .
										"items (id_parent,name,url,price,priority,shipped,price_id) values (" . $id .
										",'" . $title . "','" . $url_file . "'," . $price . "," . $dp->row["priority"] .
										",0," . $dp->row["id_parent"] . ")";
									$db->execute( $sql );
									
									$sql = "select id from " . PVS_DB_PREFIX . "items where id_parent=" . $id . " and url='" . $url_file . "' order by id desc";
									$dt->open($sql);
									if ( ! $dt->eof ) {
										$item_id = $dt->row[ 'id' ];
									}
									
									$sql =" select id_parent from " . PVS_DB_PREFIX . "filestorage_files where id_parent = " . $id . " and item_id='" . $item_id . "'";
									$ds->open($sql);
									if ( $ds->eof ) {
										$sql = "insert into " . PVS_DB_PREFIX . "filestorage_files (id_parent,item_id,url,filename1,filename2,filesize,server1,pdelete,width,height) values (" . $id . "," . $item_id . ",'" . $url_server . "','" . $url_file . "','" . $url_file . "'," . $filesize . ",0,0," . $width . "," . $height . ")";
										$db->execute( $sql );	
									}
								}
							}
							...
							{
								$sql =" select id_parent from " . PVS_DB_PREFIX . "filestorage_files where id_parent = " . $id . " and item_id='" . $dt->row["id"] . "'";
								$ds->open($sql);
								if ( $ds->eof ) {
									$sql = "insert into " . PVS_DB_PREFIX . "filestorage_files (id_parent,item_id,url,filename1,filename2,filesize,server1,pdelete,width,height) values (" . $id . "," . $dt->row["id"] . ",'" . $url_server . "','" . $url_file . "','" . $url_file . "'," . $filesize . ",0,0," . $width . "," . $height . ")";
									$db->execute( $sql );	
								}
							}
							$dp->movenext();
						...
						"='" . $url_file . "' where id=" . $id;
						$db->execute( $sql );		
																				
						if ( preg_match( "/\.jpg$|\.jpeg$|\.gif$|\.png$/i", $url_file ) and ( ! file_exists ( pvs_upload_dir() . $site_servers[$server_id] . "/" . $id . "/thumb1.jpg") or ! file_exists( pvs_upload_dir() . $site_servers[$server_id] . "/" . $id . "/thumb2.jpg")) ) {
							$tmp_file = pvs_upload_dir() . "/content/" . $tmp_folder . "/" . $url_file;
							@copy( $url,  $tmp_file);
							
							$sql =" select id_parent from " . PVS_DB_PREFIX . "filestorage_files where id_parent = " . $id . " and filename1='thumb1.jpg'";
							$ds->open($sql);
							if ( $ds->eof ) {
								pvs_photo_resize( $tmp_file, pvs_upload_dir() . $site_servers[$server_id] . "/" . $id . "/thumb1.jpg", 1 );
							}
							
							$sql =" select id_parent from " . PVS_DB_PREFIX . "filestorage_files where id_parent = " . $id . " and filename1='thumb2.jpg'";
							$ds->open($sql);
							if ( $ds->eof ) {
								pvs_photo_resize( $tmp_file, pvs_upload_dir() . $site_servers[$server_id] . "/" . $id . "/thumb2.jpg", 2 );
								pvs_publication_watermark_add( $id, pvs_upload_dir() . $site_servers[$server_id] . "/" . $id . "/thumb2.jpg" );
							}
		...
		if ( $price_license == "rights_managed" ) {
			if ( @$_POST[ 'rights_url' ] != '' and pvs_remote_check( @$_POST[ 'rights_url' ] )) {
				...
				if ($flag_url) {
					...
					$sql = "select title,price from " . PVS_DB_PREFIX . "rights_managed where id=" . ( int ) @ $_POST["rights_id"];
					$dp->open( $sql );
					if ( ! $dp->eof ) {
						$sql = "insert into " . PVS_DB_PREFIX .
							"items (id_parent,name,url,price,priority,shipped,price_id) values (" . $id .
							",'" . $dp->row["title"] . "','" . pvs_result( $url_file ) . "'," . $dp->row["price"] .
							",0,0," . ( int ) @ $_POST["rights_id"] . ")";
						$db->execute( $sql );
						
						$sql = "select id from " . PVS_DB_PREFIX . "items where id_parent=" . $id . " and url='" . pvs_result( $url_file ) . "' order by id desc";
						$dt->open($sql);
						if ( ! $dt->eof ) {
							$item_id = $dt->row[ 'id' ];
						}
					...
					$ext = strtolower( $photo_extention );
					if ( $ext == "jpeg" )
					{
						$ext = "jpg";
					}
					...
					$sql = "update " . PVS_DB_PREFIX . "media set rights_managed=" . ( int ) @ $_POST["rights_id"] . ",url_" . $ext .
					"='" . $url_file . "' where id=" . $id;
					$db->execute( $sql );
					...
					$sql =" select id_parent from " . PVS_DB_PREFIX . "filestorage_files where id_parent = " . $id . " and filename1='" . $url_file . "'";
					$ds->open($sql);
					if ( $ds->eof ) {
						$sql = "insert into " . PVS_DB_PREFIX . "filestorage_files (id_parent,item_id,url,filename1,filename2,filesize,server1,pdelete,width,height) values (" . $id . "," . $item_id . ",'" . $url_server . "','" . $url_file . "','" . $url_file . "'," . $filesize . ",0,0," . $width . "," . $height . ")";
						$db->execute( $sql );	
					...
					if ( preg_match( "/\.jpg$|\.jpeg$|\.gif$|\.png$/i", $url_file ) ) {
						$tmp_file = pvs_upload_dir() . "/content/" . $tmp_folder . "/" . $url_file;
						@copy( $url,  $tmp_file);
			...
			if ( $pvs_global_settings["photo_uploader"] == "jquery uploader" )
			{
					...
					"/" . pvs_result_file( $_POST["file_sale" . $key] ) ) )
				{
					...
					if ( strtolower( $file_extention[$key] ) != "jpg" and strtolower( $file_extention[$key] ) !=
						...
						"eps" and strtolower( $file_extention[$key] ) !=
						"jp2" and strtolower( $file_extention[$key] ) !=
						"jpf" and strtolower( $file_extention[$key] ) !=
						"zip" )
					{
						$flag[$key] = false;
					} else
	...
	if ( $price_license == "rights_managed" )
	{
		if ( $pvs_global_settings["photo_uploader"] == "usual uploader" )
		{
			...
			if ( ! preg_match( "/\.jpg$|\.jpeg$|\.gif$|\.png$|\.raw$|\.tif$|\.tiff$|\.eps$|\.jp2$|\.jpf$|\.zip$/i",
				$file_fullname[0] ) )
			{
				$flag[0] = false;
			}
		...
		if ( $pvs_global_settings["photo_uploader"] == "jquery uploader" )
		{
				...
				"/" . pvs_result_file( $_POST["file_sale100"] ) ) )
			{
				...
				if ( strtolower( $file_extention[0] ) != "jpg" and strtolower( $file_extention[0] ) !=
					...
					"eps" and strtolower( $file_extention[0] ) !=
					"jp2" and strtolower( $file_extention[0] ) !=
					"jpf" and strtolower( $file_extention[0] ) !=
					"zip" )
				{
					$flag[0] = false;
				} else
     			  		...
     			  		send: function (e, data) {
     			  			jQuery.each(data.files, function (index, file) {
          						...
          						if(flag_exist==false)
          						{
          							...
          							document.getElementById('file_sale'+bar_number).value=file.name;
          						}
        					});
     			  		},
            						...
            						if(files_massive[i]==filenam)
          							{
            							...
            							jQuery('#bar'+files_massive2[i]).css('width',progress + '%');
            							document.getElementById('file_sale'+files_massive2[i]).value=filenam;
...
function pvs_get_preview_form( $type, $flag_rights_managed, $id = 0 )
{
	global $lvideo;
	global $lpreviewvideo;
	global $laudio;
	...
	if ( pvs_is_remote_storage() and $id != 0 ) {
		$sql = "select url,filename1,filename2 from " . PVS_DB_PREFIX . "filestorage_files where item_id=0 and id_parent=" . ( int )$id;
		$dp->open( $sql );
		if ( ! $dp->eof ) {
			$flag_storage = true;
		}
	...
	if ( $flag_storage ) {
		$sql = "select server1 from " . PVS_DB_PREFIX . "media where id=" . $id;
		$dp->open( $sql );
		if ( ! $dp->eof ) {
			if ( $type == 'video' )
			{
				$remote_video = pvs_show_preview( $id, $type, 2, 1, $dp->row["server1"], $id );
				
				if ($remote_video[strlen($remote_video) - 1] == '/') {
					$remote_video = substr($remote_video, 0, strlen($remote_video) - 1);
				}
				
				$previews_remote['video_video'] = $remote_video;
				$previews_remote['video_photo'] = pvs_show_preview( $id, $type, 3, 1, $dp->row["server1"], $id );
				
				if (preg_match('/images\/icon_video\.gif$/', $previews_remote['video_photo'])) {
					$previews_remote['video_photo'] = '';
				}
			}
			if ( $type == 'audio' )
			{
				$previews_remote['audio_audio'] = pvs_show_preview( $id, $type, 2, 1, $dp->row["server1"], $id );
				$previews_remote['audio_photo'] = pvs_show_preview( $id, $type, 3, 1, $dp->row["server1"], $id );
				
				if (preg_match('/images\/icon_audio\.gif$/', $previews_remote['audio_photo'])) {
					$previews_remote['audio_photo'] = '';
				}
			}
			if ( $type == 'vector' )
			{
				$previews_remote['vector_photo'] = pvs_show_preview( $id, $type, 3, 1, $dp->row["server1"], $id );
				
				if (preg_match('/images\/icon_vector\.gif$/', $previews_remote['vector_photo'])) {
					$previews_remote['vector_photo'] = '';
				}
			}		
		}
	}
	...
	$res = '<script>
	function change_upload_url(value)
	{
		if (value == 1) 
		{
			jQuery(".upload_area").css("display","none");
			jQuery(".url_area").css("display","block");
		}
		else
		{
			jQuery(".upload_area").css("display","block");
			jQuery(".url_area").css("display","none");
		}
	}
	
	function check_youtube(form_field_id, form_field_class)
	{
		value = jQuery("#"+form_field_id).val();
		jQuery("."+form_field_class).not("#"+form_field_id).val(value);
		photo_field_id = form_field_id.replace(/_url/i, "_url2");
		video_hosting = "";
		video_id = "";
		video_check = false;
		photo_preview = "";
		
		reg_youtube =' . $pvs_regular_expressions[ 'youtube' ] . ';
		result = value.match( reg_youtube );
		if( result )
		{
			video_id = result[1];
		...
		{
			photo_preview = "https://img.youtube.com/vi/"+video_id+"/hqdefault.jpg";
			jQuery(".photo-preview-field").val(photo_preview);
			jQuery(".preview_youtube").css("display","inline");
			jQuery(".preview_vimeo").css("display","none");
			jQuery(".upload_photo_area").html("<img src=\""+photo_preview+"\" style=\"max-width:120px;margin-top:3px\">");
			jQuery(".upload_video_area").html("<iframe width=\"' . $pvs_global_settings["ffmpeg_video_width"] . '\" height=\"' . $pvs_global_settings["ffmpeg_video_height"] . '\" src=\"https://www.youtube.com/embed/"+video_id+"\" frameborder=\"0\" allow=\"autoplay; encrypted-media\" allowfullscreen></iframe>");
		}
		
		reg_vimeo=' . $pvs_regular_expressions[ 'vimeo' ] . ';
		result = value.match( reg_vimeo );
		...
		if(video_hosting == "vimeo")
		{

			jQuery.getJSON("https://www.vimeo.com/api/v2/video/" + video_id + ".json?callback=?", { format: "json" }, function (data) {
                    photo_preview = data[0].thumbnail_large;
                    jQuery(".photo-preview-field").val(photo_preview);
					jQuery(".preview_youtube").css("display","none");
					jQuery(".preview_vimeo").css("display","inline");
					jQuery(".upload_photo_area").html("<img src=\""+photo_preview+"\" style=\"max-width:120px;margin-top:3px\">");
                });
                
			jQuery(".upload_video_area").html("<iframe src=\"https://player.vimeo.com/video/"+video_id+"?autoplay=0&loop=0\" width=\"' . $pvs_global_settings["ffmpeg_video_width"] . '\" height=\"' . $pvs_global_settings["ffmpeg_video_height"] . '\" frameborder=\"0\" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>");
		}
		
		
		
		if (video_check == false) {
			check_remote(form_field_id,form_field_class,"(mp4)$", "upload_video_area");
			jQuery(".photo-preview-field").val("");
			jQuery(".preview_youtube").css("display","none");
			jQuery(".preview_vimeo").css("display","none");
			jQuery(".upload_photo_area").html("");
			jQuery(".upload_video_area").html("");
		}
	...
	if ( pvs_is_user_admin () and $type != 'photo') {
		if ( ! $flag_storage ) {
			$check1 = 'checked';
			$check2 = '';
		} else {
		...
		$res .= "<h4>" . pvs_word_lang("preview") . "</h4><input type='radio' name='upload_url" . $rights_field . "' id='upload_url1" . $rights_field . "' value='0' " . $check1 . " onClick='change_upload_url(0);'>&nbsp;<label for='upload_url1" . $rights_field . "'>" . pvs_word_lang("upload") . "</label>&nbsp;&nbsp;&nbsp;&nbsp;<input type='radio' name='upload_url" . $rights_field . "' id='upload_url2" . $rights_field . "' " . $check2 . " value='1'  onClick='change_upload_url(1);'>&nbsp;<label for='upload_url2" . $rights_field . "'>" . pvs_word_lang("URL") . "</label>";
	}
	...
	if ( $type == "video" )
	{
		...
		$res .= "<tr class='snd'><td colspan='4'><small>(" . pvs_word_lang( "size" ) . " < " .
			$lvideo . "Mb.)</small></td></tr>";
		...
		if ( ! $pvs_global_settings["ffmpeg"] )
		{
			...
			if ( $pvs_global_settings["video_uploader"] == "usual uploader" )
			{
				$res .= "<div class='upload_area'><input class='ibox form-control' name='preview" . $rights_field .
					"' type='file' style='width:400px'></div>";
			}

			if ( $pvs_global_settings["video_uploader"] == "jquery uploader" )
			{
				$res .= "<div class='upload_area'><span class=\"btn btn-success fileinput-button\">
        					<i class=\"icon-plus icon-white\" style=\"float:left\"></i> " .
					pvs_word_lang( "select files" ) . "...
        					<!-- The file input field used as target for the file upload widget -->
					...
					"' name='file_sale" . $number_field0 . "' value=''>
    					...
    					<div id=\"files" . $number_field0 . "\" class=\"files\"></div></div>
    					";

			}
			
			if ( pvs_is_user_admin () ) {
				$res .= "<div class='url_area' style='display:none'><div class='clearfix'><input class='ibox form-control video-preview-field'  id='preview_url" . $rights_field .
					"'  name='preview_url" . $rights_field .
					"' type='text' style='width:600px;float:left' onKeyup='check_youtube(\"preview_url" . $rights_field .
					"\", \"video-preview-field\")'  value='" . @$previews_remote['video_video'] . "'>&nbsp;&nbsp;<i class='fa fa-youtube-square preview_youtube' style='font-size:33px;display:none'></i><i class='fa fa-vimeo-square preview_vimeo' style='font-size:33px;display:none'></i></div><div class='upload_video_area' style='margin-top:3px'><span class='smalltext'>URL: http://www.domain.com/video.mp4<br>Youtube: https://www.youtube.com/watch?v=7G_fYgW5Tys<br>Vimeo: https://vimeo.com/14362497</span></div></div>";
			}

			$res .= "<div class='upload_area'><span class='smalltext'>(*.mp4 or *.flv " . pvs_word_lang( "size" ) .
				" < " . $lpreviewvideo . "Mb.)</span></div></td></tr>";
			...
			if ( $pvs_global_settings["video_uploader"] == "usual uploader" )
			{
				$res .= "<div class='upload_area'><input class='ibox form-control' name='preview2" . $rights_field .
					"' type='file' style='width:400px'></div>";
			}
			...
			if ( $pvs_global_settings["video_uploader"] == "jquery uploader" )
			{
				$res .= "<div class='upload_area'><span class=\"btn btn-success fileinput-button\">
					...
					"' name='file_sale" . $number_field1 . "' value=''>
    					...
    					<div id=\"files" . $number_field1 . "\" class=\"files\"></div></div>
    					";

			}
			
			if ( pvs_is_user_admin () ) {
				$res .= "<div class='url_area' style='display:none'><input class='ibox form-control photo-preview-field' id='preview_url2" . $rights_field .
					"' name='preview_url2" . $rights_field .
					"' type='text' style='width:600px' value='" . @$previews_remote['video_photo'] . "' onKeyup='check_remote(\"preview_url2" . $rights_field .
					"\", \"photo-preview-field\",\"(jpg|jpeg)$\", \"upload_photo_area\")' ><div class='upload_photo_area'><span class='smalltext'>URL: http://www.domain.com/photo.jpg</span></div></div>";
			}

			$res .= "<div class='upload_area'><span class='smalltext'>(*.jpg,*.jpeg)</span></div></td></tr>";
		} else {
			if ( ! $flag_rights_managed )
			{
				$res .= "<tr><td colspan='4'>
				<div class='upload_area'><b>" . pvs_word_lang( "Generate video preview from" ) .
					":</b><br><select name='generation' style='width:400px' class='ibox form-control'>";
				...
				$res .= "<tr><th colspan=4>";
				if ( pvs_is_user_admin () ) {
					$res .= "<div class='url_area' style='display:none'><b>" . pvs_word_lang( "preview" ) . " " .
				pvs_word_lang( "video" ) . ":</b><br><div class='clearfix'><input class='ibox form-control video-preview-field'  id='preview_url" . $rights_field .
						"'  name='preview_url" . $rights_field .
						"' type='text' style='width:600px;float:left' onKeyup='check_youtube(\"preview_url" . $rights_field .
						"\", \"video-preview-field\")'  value='" . @$previews_remote['video_video'] . "'>&nbsp;&nbsp;<i class='fa fa-youtube-square preview_youtube' style='font-size:33px;display:none'></i><i class='fa fa-vimeo-square preview_vimeo' style='font-size:33px;display:none'></i></div><div class='upload_video_area' style='margin-top:3px'><span class='smalltext'>URL: http://www.domain.com/video.mp4<br>Youtube: https://www.youtube.com/watch?v=7G_fYgW5Tys<br>Vimeo: https://vimeo.com/14362497</span></div></div>";
				}
				...
				if ( pvs_is_user_admin () ) {
					$res .= "<br><div class='url_area' style='display:none'><b>" . pvs_word_lang( "preview" ) . " " .
				pvs_word_lang( "photo" ) . ":</b><br><input class='ibox form-control photo-preview-field' id='preview_url2" . $rights_field .
					"' name='preview_url2" . $rights_field .
					"' type='text' style='width:600px' value='" . @$previews_remote['video_photo'] . "' onKeyup='check_remote(\"preview_url2" . $rights_field .
					"\", \"photo-preview-field\",\"(jpg|jpeg)$\", \"upload_photo_area\")' ><div class='upload_photo_area'><span class='smalltext'>URL: http://www.domain.com/photo.jpg</span></div></div>";
				}
	...
	if ( $type == "audio" )
	{
		if ( $pvs_global_settings["sox"] == 0 )
		{
			...
			if ( pvs_is_user_admin () ) {
				$res .= "<div class='url_area' style='display:none'><div class='clearfix'><input class='ibox form-control audio-preview-field'  id='preview_url" . $rights_field .
					"'  name='preview_url" . $rights_field .
					"' type='text' style='width:600px;float:left' onKeyup='check_remote(\"preview_url" . $rights_field .
					"\", \"audio-preview-field\",\"(mp3)$\", \"upload_audio_area\")'  value='" . @$previews_remote['audio_audio'] . "'></div><div class='upload_audio_area' style='margin-top:3px'><span class='smalltext'>URL: http://www.domain.com/audio.mp3</span></div></div>";
			}

			$res .= "<div class='upload_area'><span class='smalltext'>(*.mp3 " . pvs_word_lang( "size" ) . " < " . $lpreviewaudio .
				"Mb.)</span></div></td></tr>";
		} else
		{
			if ( ! $flag_rights_managed )
		...
		if ( pvs_is_user_admin () ) {
			$res .= "<div class='url_area' style='display:none'><input class='ibox form-control photo-preview-field' id='preview_url2" . $rights_field .
					"' name='preview_url2" . $rights_field .
					"' type='text' style='width:600px' value='" . @$previews_remote['audio_photo'] . "' onKeyup='check_remote(\"preview_url2" . $rights_field .
					"\", \"photo-preview-field\",\"(jpg|jpeg)$\", \"upload_photo_area\")' ><div class='upload_photo_area'><span class='smalltext'>URL: http://www.domain.com/photo.jpg</span></div></div>";
		}
	...
	if ( $type == "vector" )
	{
		...
		if ( pvs_is_user_admin () ) {
			$res .= "<div class='url_area' style='display:none'><input class='ibox form-control photo-preview-field' id='preview_url2" . $rights_field .
					"' name='preview_url2" . $rights_field .
					"' type='text' style='width:600px' value='" . @$previews_remote['vector_photo'] . "' onKeyup='check_remote(\"preview_url2" . $rights_field .
					"\", \"photo-preview-field\",\"(jpg|jpeg)$\", \"upload_photo_area\")' ><div class='upload_photo_area'><span class='smalltext'>URL: http://www.domain.com/photo.jpg</span></div></div>";
		}

		$res .= "<div class='upload_area'><span class='smalltext'>(*.jpg,*.jpeg or *.zip archive of jpgs)</span></div></td></tr>";
	}
	
	$res .= "</table>";
	
	if ( pvs_is_user_admin () and $flag_storage ) {
		$res .= "<script>change_upload_url(1);</script>";
	}
...
//CSV parcer
function pvs_parcer_csv($file_name, $separator=',', $quote='"') {
