...
function pvs_show_rating( $id, $rating )
{
	...
	$rating_text2 = "<script>
				...
				<div class='box_rating' id='star" . $id . "' style='display:inline'></div>";
...
function pvs_show_related_items( $id, $type )
{
	...
	if ( ! $pvs_global_settings["elasticsearch"] or ! $pvs_global_settings["elasticsearch_related"] ) {
		$sqlkey = "";
			...
			if ( ! $ds->eof )
			{
				...
				while ( ! $ds->eof )
				{
					if ( $tt < $pvs_global_settings["related_items_quantity"] )
					{
						...
						if ($type == 'show') {
							if ( file_exists ( get_stylesheet_directory(). '/item_related.php' ) ) {
								include( get_stylesheet_directory(). '/item_related.php' );
							} else {
								if ( file_exists ( PVS_PATH . 'templates/item_related.php' ) ) {
									include( PVS_PATH . 'templates/item_related.php' );
								}
							}
						}
		...
		//Sorting
		$params["sort"] = array("data" => array("order" => "desc"));
		
		/*
		if ( count($keywords) > 1) {
			for ( $i = 1; $i < count( $keywords ); $i++ ) {
				$filters[] = array("term" => array("keywords" => $keywords[$i]));
			}
		}
		*/
		
		$params["query"] = array("bool" => array("must" => array("multi_match" => array("query"=>$keywords[0],
		...
		//Paging
		$params["size"] = $pvs_global_settings["related_items_quantity"];
		$params["from"] = 0;
		...
		if ( @$response->hits->total->value > 0) {
			foreach ( @$response->hits->hits as $key => $value ) {
				$sql = "select  id, media_id, title, server1, url, author, description from " .
						PVS_DB_PREFIX . "media where id <> " . ( int )$id . " and id = " . @$value->_id;
		...
		while ( ! $dp->eof )
		{
			...
			if ( ! $dx->eof )
			{
				...
				if ( ! $dt->eof )
				{
					...
					if ($type == 'show') {
						if ( file_exists ( get_stylesheet_directory(). '/item_related_prints.php' ) ) {
							include( get_stylesheet_directory(). '/item_related_prints.php' );
						} else {
							if ( file_exists ( PVS_PATH . 'templates/item_related_prints.php' ) ) {
								include( PVS_PATH . 'templates/item_related_prints.php' );
							}
						}
					}
...
function pvs_show_favorite( $id )
{
	...
	$pvs_theme_content[ 'add_to_favorite_link2' ] = "show_lightbox(" . ( int )$id . ",'" . site_url() . "')";
}
//End. The function shows a publication's add to favority button



//The function shows 'add to package' button
function pvs_show_package( $id, $print_id )
{
	global $db;
	global $dr;
	global $pvs_theme_content;

	$pvs_theme_content[ 'add_to_package_link' ] = "javascript:show_package(" . ( int )$id . ", " . $print_id . ")";
	$pvs_theme_content[ 'add_to_package_link2' ] = "show_package(" . ( int )$id . ", " . $print_id . ")";
}
//End. The function shows 'add to package' button
		...
		for ( $i = 0; $i < count( $keywords ); $i++ )
		{
			...
			if ( $keywords[$i] != "" )
			{
				$kw .= "<div style='margin-bottom:3px'><input type='checkbox' name='s_" .
					...
					$kw_lite .= "<a href='" . pvs_catalog_url() . urlencode($keywords[$i]) . "' class='kw'>" . $keywords[$i] . "</a> ";
			}
		}
	}
...
function pvs_show_navigation( $id, $type, $print_id = 0, $print_type = '' )
{
	...
	*/
	$sql = "select id,title from " . PVS_DB_PREFIX .
		"media where id < " . ( int )$id .
		$com_print . " and published=1 order by id desc limit 0,1";
	$dr->open( $sql );
	if ( ! $dr->eof )
	{
		if ( $print_id != 0 )
	...
	if ($pvs_global_settings['show_colors']) {
		...
		while (!$dd->eof) {
			if ($colors_count < $pvs_global_settings['colors_number']) {
				$colors .= '<a href="' . pvs_catalog_url(true) . '?color=' . $dd->row['color'] . '&s' . $type . '=1" style="background-color:#' . $dd->row['color'] . ';width:10px">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</a>&nbsp;';
			}
			$colors_count ++;
			$dd -> movenext();
		}	
