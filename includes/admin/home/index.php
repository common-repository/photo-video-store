...
//Add
if ( @$_REQUEST["action"] == 'add' and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-home-add' ) )
{
	include ( "components_add.php" );
}

//Change
if ( @$_REQUEST["action"] == 'change' and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-home-change' ) )
{
	include ( "components_change.php" );
}

//Delete
if ( @$_REQUEST["action"] == 'delete' and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-home-delete' ) )
{
...
?>&action=delete&id=<?php
		echo $rs->row["id"]
?>&_wpnonce=<?php echo wp_create_nonce('pvs-home-delete'); ?>' onClick="return confirm('<?php
		echo pvs_word_lang( "delete" )
?>?');"><?php
		echo pvs_word_lang( "delete" )
?></a>
...
<input type="hidden" name="id" value="<?php
		echo $_GET["id"]
?>">
<?php wp_nonce_field( 'pvs-home-change' ); ?>
<?php
	} else
	{
		$title = "";
...
<?php wp_nonce_field( 'pvs-home-add' ); ?>
<?php
	}
?>
...
<?php
	...
	pvs_build_menu_admin( 0, $category, 2, $iid );
	echo ( $itg );
?>
</select>
</div>
