...
while ( ! $rs->eof ) {
	...
	if ( ! is_writeable(pvs_upload_dir() . $rs->row["url"] ) ) {
		echo('<div class="alert alert-danger" role="alert">Error! The directory <b>' . pvs_upload_dir() . $rs->row["url"] . '</b>  is <b>not writable</b>. The script cannot upload any files there.</p></div>');
	}
