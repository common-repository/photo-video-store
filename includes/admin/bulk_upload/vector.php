...
<h1><?php echo pvs_word_lang( "FTP vector uploader" )?></h1>
...
<?php wp_nonce_field( 'pvs-vector-upload' ); ?>
...
?>
		...
		<div class="form_field">
			...
			<select class="form-control" name="path" style="width:350px;margin-top:2px" onChange="location.href='<?php echo(pvs_plugins_admin_url('bulk_upload/index.php'));?>&d=4&path='+this.value">
				<option value=""><?php echo($pvs_global_settings["vectorpreupload"] );?></option>
				<?php
				$dir = opendir( pvs_upload_dir() . $pvs_global_settings["vectorpreupload"] );
				while ( $file = readdir( $dir ) ) {
					if ( $file <> "." and $file <> ".." and is_dir ( pvs_upload_dir() . $pvs_global_settings["vectorpreupload"] . $file ) ) {
						...
						?>
						<option value="<?php echo($file);?>" <?php echo($sel);?>><?php echo($pvs_global_settings["vectorpreupload"] . $file);?></option>
						<?php
					}
				}
				closedir( $dir );
...
?>
							<script>
								function set_license(value)
								{
									if(value==1)
									{
										jQuery('.box_license2').css('display','none');
										jQuery('.box_license1').css('display','block');
									}
									else
									{
										jQuery('.box_license2').css('display','block');
										jQuery('.box_license1').css('display','none');
									}
								}
							</script>
...
$dir = opendir( pvs_upload_dir() . $pvs_global_settings["vectorpreupload"]  . $path );
while ( $file = readdir( $dir ) ) {
...
for ( $i = 0; $i < $upload_limit; $i++ ) {
	...
	$size = getimagesize( pvs_upload_dir() . $pvs_global_settings["vectorpreupload"]  . $path .
		'/' . $afiles[$i], $info );
	if ( isset( $info["APP13"] ) ) {
...
?>/admin_photo_preview/?file=<?php echo $afiles[$i] ?>&type=vector&path=<?php echo $path ?>"><input name="file<?php echo $i
