...
for ( $j = 0; $j < $upload_limit; $j++ ) {
	...
	if ( $flag_upload == true ) {
		...
		$previewphoto = $pvs_global_settings["vectorpreupload"]  . $path . pvs_result( $_POST["previewphoto" .
			$j] );
		...
		if ( $rights_managed == 0 ) {
			...
			while ( ! $ds->eof )
			{
				if ( $ds->row["shipped"] != 1 )
				{
					...
					{
						$vector = $pvs_global_settings["vectorpreupload"]  . $path . pvs_result( $_POST["file" .
							$ds->row["id_parent"] . "_" . $j] );
