...
for ( $j = 0; $j < $pvs_global_settings["bulk_upload"]; $j++ ) {
	...
	if ( $flag_upload == true ) {
		...
		$previewaudio = $pvs_global_settings["audiopreupload"]   . $path . pvs_result( $_POST["previewaudio" .
			$j] );
		$previewphoto = $pvs_global_settings["audiopreupload"]  . $path . pvs_result( $_POST["previewphoto" .
			$j] );
		...
		if ( $rights_managed == 0 ) {
			...
			while ( ! $ds->eof )
			{
				if ( $ds->row["shipped"] != 1 )
				{
					...
					{
						$audio = $pvs_global_settings["audiopreupload"]  . $path . pvs_result( $_POST["file" .
							$ds->row["id_parent"] . "_" . $j] );
			...
			while ( ! $ds->eof )
			{
				...
				{
					$video = $pvs_global_settings["audiopreupload"]  . $path . pvs_result( $_POST["file" .
						$ds->row["id"] . "_" . $j] );
