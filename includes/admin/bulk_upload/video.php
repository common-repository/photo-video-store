...
?>
		...
		<div class="form_field">
			...
			<select class="form-control" name="path" style="width:350px;margin-top:2px" onChange="location.href='<?php echo(pvs_plugins_admin_url('bulk_upload/index.php'));?>&d=2&path='+this.value">
				<option value=""><?php echo($pvs_global_settings["videopreupload"] );?></option>
				<?php
				$dir = opendir( pvs_upload_dir() . $pvs_global_settings["videopreupload"] );
				while ( $file = readdir( $dir ) ) {
					if ( $file <> "." and $file <> ".." and is_dir ( pvs_upload_dir() . $pvs_global_settings["videopreupload"] . $file ) ) {
						...
						?>
						<option value="<?php echo($file);?>" <?php echo($sel);?>><?php echo($pvs_global_settings["videopreupload"] . $file);?></option>
						<?php
					}
				}
				closedir( $dir );
...
if ( file_exists( pvs_upload_dir() . $pvs_global_settings["videopreupload"]  . $path ) ) {
	$dir = opendir( pvs_upload_dir() . $pvs_global_settings["videopreupload"]  . $path );
	while ( $file = readdir( $dir ) ) {
		if ( $file <> "." && $file <> ".." ) {
