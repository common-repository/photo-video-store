...
$afiles=pvs_parcer_csv(pvs_upload_dir().$pvs_global_settings["videopreupload"]."/videos.csv");
...
for ( $j = 1; $j < count( $afiles ); $j++ ) {
	...
	$csv_video_preview = @$afiles[$j][2];
	if ( $csv_video_preview{0} == '/') {
		$csv_video_preview = substr($csv_video_preview,1);
	}
	...
	if ( file_exists ( pvs_upload_dir().$pvs_global_settings["videopreupload"]."/" . $csv_path ) ) {
		...
		$previewvideo = $pvs_global_settings["videopreupload"] . $csv_video_preview;
		$previewphoto = $pvs_global_settings["videopreupload"] . $csv_photo_preview;
		...
		while ( ! $ds->eof )
		{
			if ( $ds->row["shipped"] != 1 )
			{
				...
				if ( $flag )
				{
					$video = $pvs_global_settings["videopreupload"] . $csv_path;
					copy( pvs_upload_dir() . $video, pvs_upload_dir() . $site_servers[$site_server_activ] . "/" . $folder . "/" . $csv_filename );
					...
					if ( $ds->row["thesame"] > 0 )
					{
						$sql = "select * from " . PVS_DB_PREFIX . "video_types where id_parent<>" . $ds->
		...
		if ( ! $pvs_global_settings["ffmpeg"] ) {
			//Video preview
			if ( file_exists ( pvs_upload_dir().$pvs_global_settings["videopreupload"]."/" . $csv_video_preview ) ) {
				$vp = $site_servers[$site_server_activ] . "/" . $folder . "/thumb.mp4";
				copy( pvs_upload_dir() . $previewvideo, pvs_upload_dir() . $vp );
			}
	
			//Photo preview
			if ( file_exists ( pvs_upload_dir().$pvs_global_settings["videopreupload"]."/" . $csv_photo_preview ) ) {
			...
			pvs_generate_video_preview( pvs_upload_dir() . $site_servers[$site_server_activ] . "/" . $folder . "/" . $csv_filename, 0, 0 );
		}
