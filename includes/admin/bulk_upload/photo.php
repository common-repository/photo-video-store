	...
	<br>
	  ...
	  <div class="group_settings group_files">
		<div class="form_field">
			...
			<p>If you want to upload <b>additional formats</b> (*.gif,*.png,*.raw,*.tiff,*.eps,*.jp2,*.jpf,*.zip) the files must have the same names as *.jpg.</p>
		</div>
		
		
		<div class="form_field">
			<span><b><?php echo ( pvs_word_lang( "Select folder" ) );?>:</b></span>	
			<select class="form-control" name="path" style="width:350px;margin-top:2px" onChange="location.href='<?php echo(pvs_plugins_admin_url('bulk_upload/index.php'));?>&d=1&path='+this.value">
				<option value=""><?php echo($pvs_global_settings["photopreupload"] );?></option>
				<?php
				$dir = opendir( pvs_upload_dir() . $pvs_global_settings["photopreupload"] );
				while ( $file = readdir( $dir ) ) {
					if ( $file <> "." and $file <> ".." and is_dir ( pvs_upload_dir() . $pvs_global_settings["photopreupload"] . $file ) ) {
						...
						?>
						<option value="<?php echo($file);?>" <?php echo($sel);?>><?php echo($pvs_global_settings["photopreupload"] . $file);?></option>
						<?php
					}
				}
				closedir( $dir );
...
$dir = opendir( pvs_upload_dir() . $pvs_global_settings["photopreupload"]  . $path );
while ( $file = readdir( $dir ) ) {
	if ( $file <> "." && $file <> ".." ) {
...
?>
		...
		function selectall() {
			if(document.getElementById("allphotos").checked)
			{
				...
				{
					jQuery(".photocheck").attr("checked",true);
				}
			}
			else
			{
				for(i=0;i<<?php echo count( $afiles )?>;i++)
				{
					jQuery(".photocheck").attr("checked",false);
				}
			}
		}
...
?>
		...
		<div class="form_field">
			<?php
if ( isset($_GET["quantity"]) and ( int )$_GET["quantity"] != 0 ) {
	echo ( pvs_paging( $n, $str, ( int )$_GET["quantity"], 7, pvs_plugins_admin_url('bulk_upload/index.php') . '&d=1&quantity=' . ( int )$_GET["quantity"],
		"&d=1" ) );
	$kolvo = ( int )$_GET["quantity"];
} else
{
	$kolvo = 100000000000;
}
...
for ( $i = 0; $i < count( $afiles ); $i++ ) {
	if ( $i > $kolvo * ( $str - 1 ) - 1 and $i < $kolvo * $str ) {
		...
		$size = getimagesize( pvs_upload_dir() . $pvs_global_settings["photopreupload"] . $path . '/' .  $afiles[$i], $info );
			
			//$clarifai_files .= 'clarifai_files["keywords' . $i . '"] = "' . site_url() . '/admin_photo_preview/?file=' . $afiles[$i] . '&type=photo";';
			$clarifai_files .= 'clarifai_files["keywords' . $i . '"] = "' . site_url() . '/wp-content/uploads/photopreupload/' . $path . '/' . $afiles[$i] . '";';
		...
		$exif_info = @exif_read_data( pvs_upload_dir() . $pvs_global_settings["photopreupload"] . $path . '/' . $afiles[$i], 0, true );
	
		
		
		if ( isset( $exif_info["IFD0"]["Title"] ) and $exif_info["IFD0"]["Title"] != ''  ) {
			$title = str_replace(',amp,','and', preg_replace("/[^a-z0-9 ,;\.\-]*/iU","",$exif_info["IFD0"]["Title"]));
			//$title = mb_convert_encoding($title, 'UTF-8', pvs_get_encoding($title));
		}	
		
		if ( isset( $exif_info["IFD0"]["ImageDescription"] ) and $exif_info["IFD0"]["ImageDescription"] != ''  ) {
			$description = str_replace(',amp,','and', preg_replace("/[^a-z0-9 ,;\.\-]*/iU","",$exif_info["IFD0"]["ImageDescription"]));
			//$description = mb_convert_encoding($description, 'UTF-8', pvs_get_encoding($description));
		}
		
		if ( isset( $exif_info["IFD0"]["Keywords"] ) and $exif_info["IFD0"]["Keywords"] != ''  ) {
			$keywords = str_replace(',amp,','and', preg_replace("/[^a-z0-9 ,;\.\-]*/iU","",$exif_info["IFD0"]["Keywords"]));
			//$keywords = mb_convert_encoding($keywords, 'UTF-8', pvs_get_encoding($keywords));
		}
...
?>/admin_photo_bulk/?file=<?php echo $afiles[$i] ?>&type=photo&path=<?php echo $path ?>"><input name="file<?php echo $i
...
?></small></div>
				...
				if ( file_exists( pvs_upload_dir() . $pvs_global_settings["photopreupload"] . "/" . $path . '/' .
					$filename . ".tiff" ) )
				{
?>
								<div style='margin-bottom:3px'><small><?php
...
?></textarea></div>
					<div style="margin-top:3px"><textarea class='textarea' id="keywords<?php echo $i
?>" name="keywords<?php echo $i
