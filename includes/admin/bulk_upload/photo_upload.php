...
if (isset($_POST["path"])) {
	$path = str_replace("/","", str_replace("\\","",$_POST["path"])) . "/";
}
...
for ( $j = 0; $j < count( $afiles ); $j++ ) {
	if ( isset( $_POST["f" . $j] ) ) {
		...
		if ( $_POST["file" . $j] != "" ) {
			...
			$photo = $pvs_global_settings["photopreupload"] . $path . '/' . pvs_result( $_POST["file" .
				$j] );
			...
			foreach ( $photo_formats as $key => $value )
			{
				...
				if ( $value == "tiff" )
				{
					if ( file_exists( pvs_upload_dir() . $pvs_global_settings["photopreupload"] . $path . "/" .
						$filename . ".tif" ) )
					{
						copy( pvs_upload_dir() . $pvs_global_settings["photopreupload"]  . $path . '/' . $filename .
				...
				{
					@unlink( pvs_upload_dir() . $pvs_global_settings["photopreupload"]  . $path . '/' . $filecopy );
				}
			}
