...
for ( $j = 0; $j < $pvs_global_settings["bulk_upload"]; $j++ ) {
	...
	if ( $flag_upload == true ) {
		...
		$previewvideo = $pvs_global_settings["videopreupload"]  . $path . @$_POST["previewvideo" .
			$j];
		$previewphoto = $pvs_global_settings["videopreupload"]  . $path . @$_POST["previewphoto" .
			$j];
		...
		if ( $rights_managed == 0 ) {
			...
			while ( ! $ds->eof )
			{
				if ( $ds->row["shipped"] != 1 )
				{
					...
					{
						$video = $pvs_global_settings["videopreupload"]  . $path . pvs_result( $_POST["file" .
							$ds->row["id_parent"] . "_" . $j] );
