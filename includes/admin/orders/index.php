...
//Comment change
if ( @$_REQUEST["action"] == 'comments_change' and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-comments-change' ) )
{
...
//Delete
if ( @$_REQUEST["action"] == 'order_delete' and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-order-delete' ) )
{
	include ( "order_delete.php" );
}

//Restore
if ( @$_REQUEST["action"] == 'restore' and wp_verify_nonce( @$_REQUEST['_wpnonce'], 'pvs-order-restore' ) )
{
	include ( "restore.php" );
}
...
<a class="btn btn-success toright" href="<?php echo (site_url( ) );?>/orders-export-csv/?_wpnonce=<?php echo wp_create_nonce('pvs-orders-export-csv'); ?>"><i class="icon-th icon-white fa fa-file-text"></i>&nbsp; <?php echo pvs_word_lang( "export as csv file" )?></a>

<a class="btn btn-success toright" href="<?php echo (site_url( ) );?>/orders-export-xls/?_wpnonce=<?php echo wp_create_nonce('pvs-orders-export-xls'); ?>"><i class="icon-th-list icon-white fa fa-file-excel-o"> </i>&nbsp; <?php echo pvs_word_lang( "export as xls file" )?></a>
...
function pvs_order_status(value) 
{
	jQuery.ajax({
		type:'POST',
		url:ajaxurl,
		data:'action=pvs_order_status&id=' + value + '&_wpnonce=<?php echo wp_create_nonce('pvs-order-status'); ?>',
...
function pvs_order_shipping_status(value) 
{
	jQuery.ajax({
		type:'POST',
		url:ajaxurl,
		data:'action=pvs_order_shipping_status&id=' + value + '&_wpnonce=<?php echo wp_create_nonce('pvs-shipping-status'); ?>',
		success:function(data){
			if(data.charAt(data.length-1) == '0')
			{
				data = data.substring(0,data.length-1)
...
function publications_select_all(sel_form) {
    if(sel_form.selector.checked)
   	{
        jQuery("input:checkbox", sel_form).attr("checked",true);
    }
    else
    {
        jQuery("input:checkbox", sel_form).attr("checked",false);
    }
}
</script>
...
<?php wp_nonce_field( 'pvs-order-delete' ); ?>
...
<?php
	$property_value = pvs_get_admin_notifications( 'orders', 'value' );
	while ( ! $rs->eof ) {
		...
		$cl3 = "";
		$cl_script = "";
		if ( $rs->row["id"] > $property_value and $property_value > 0 ) {
