...
if ( $rs->row["contacts"] == 0 ) {
	...
	if ( $rs->row["rights_managed"] == 0 )
	{
			...
			$folder ) )
		{
			...
			while ( ! $dd->eof )
			{
				...
				while ( ! $dr->eof )
				{
					$sql = "select id,name,url,price,shipped,contacts from " . PVS_DB_PREFIX .
					...
					if ( ! $ds->eof )
					{
						...
						} else
						{
							...
							if ( $ds->row["contacts"] == 1 )
							{
								...
								row["name"] ) . "</td><td class='hidden-xs hidden-sm'>&mdash;</td><td colspan='2' nowrap><a href='" .
	...
	} else
	{
		...
		if ( ! $ds->eof )
		{
			...
			if ( $file_name != "" ) {
				if ( file_exists( pvs_upload_dir() . pvs_server_url( $rs->row["server1"] ) . "/" . $folder . "/" . $file_name ) ) {
					$video_filesize = filesize( pvs_upload_dir() . pvs_server_url( $rs->row["server1"] ) . "/" . $folder . "/" . $file_name );
				} else {
