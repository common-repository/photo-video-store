...
if ( ! $dq->eof ) {
	...
	if ( ! $dd->eof ) {
		...
		for ( $i = 1; $i < 11; $i++ ) {
			...
			jQuery('#colorpicker" . $dn->row["id"] . "').ColorPicker({
		...
		jQuery('#colorpicker" . $dn->row["id"] .
							" div').css('backgroundColor', '#' + hex);
		jQuery('#property" . $i . "').val('#' + hex);
						...
						while ( ! $ds->eof )
						{
							...
							if ( $flag_show )
							{
								...
								if ( $dn->row["type"] == "frame" )
								{
									...
									$preloaded_frames .= "jQuery.preloadImages('" . pvs_plugins_url() .
								...
								if ( $dn->row["type"] == "background" )
								{
									...
									$preloaded_tshirts .= "jQuery.preloadImages('" . pvs_plugins_url() .
...
if ( isset( $dq->row["in_stock"] ) ) {
	...
	if ( $dq->row["in_stock"] == -1 ) {
		...
		$in_stock_text = '<font style="color:red">' . pvs_word_lang( "Not in stock" ) .
			'</font><script>jQuery(document).ready(function() {jQuery(".add_to_cart").css("display","none")});</script>';
	} else {
...
if ( $in_stock != 0 ) {
	$prints_content .= '<div class="input-group" style="width:110px">
		...
		<button href="javascript:quantity_change(-1,' . $in_stock .
		')" class="btn btn-default" style="padding-left:10px;padding-right:10px"><i class="fa fa-minus" aria-hidden="true"></i></button>
				  </span>
				  <input id="quantity"  name="quantity" type="text" class="form-control" value="' .
		$quantity . '" style="padding-left:7px;padding-right:7px;width:50px">
				  <span class="input-group-btn">
		<button href="javascript:quantity_change(1,' . $in_stock .
		')" class="btn btn-default" style="padding-left:10px;padding-right:10px"><i class="fa fa-plus" aria-hidden="true"></i></button>
				  </span>     
