...
if ( pvs_check_password_publication( get_query_var('pvs_id') ) ) {
	...
	if ( ! $rs->eof ) {
		...
		if ( pvs_is_remote_storage() ) {
			$sql = "select filename1,filename2,url,item_id,filesize,width,height from " . PVS_DB_PREFIX .
			...
			while ( ! $ds->eof )
			{
				...
				} else
				{
					...
					if ( $ds->row["filename1"] == "thumb2.jpg" )
					{
		...
		if ( $preview_items != "" ) {
			$preview_items = "<style>#galleria{height:320px;width:100%}</style><div id='galleria'>" . $preview_items .
	...
	<span class="sr-only">Next</span>
		...
		if ( preg_match( "/adult_content/", $vector_preview ) ) {
			$pvs_theme_content[ 'image' ] = pvs_show_preview( $rs->row["id"], "vector", 3, 0, $rs->row["server1"], $rs->row["id"] );
		}
		...
		if ( preg_match( "/icon_vector/", $vector_preview ) ) {
			...
			if ( $pvs_global_settings["zoomer"] and ! preg_match( "/icon_vector/", $vector_preview ) )
			{
				...
				$pvs_theme_content[ 'image' ] = $vector;			
			}
		}
		...
		if ( file_exists ( get_stylesheet_directory(). '/item_vector.php' ) ) {
			require_once( get_stylesheet_directory(). '/item_vector.php' );
		} else {
			if ( file_exists ( PVS_PATH . 'templates/item_vector.php' ) ) {
				require_once( PVS_PATH . 'templates/item_vector.php' );
			}
		}
	}
}
