...
if ( pvs_check_password_publication( get_query_var('pvs_id') ) ) {
	$type = 'item';
	if ( file_exists ( PVS_PATH . 'templates/item_protected.php' ) ) {
		require_once( PVS_PATH . 'templates/item_protected.php' );
	}	
} else {
	$sql = "update " . PVS_DB_PREFIX . "media set viewed=viewed+1 where id=" . ( int )get_query_var('pvs_id');
	$db->execute( $sql );
	
	pvs_elastic_update( ( int )get_query_var('pvs_id') );
	
	$sql = "select id,title,data,published,description,featured,keywords,author,viewed,userid,watermark,free,downloaded,rating,server1,google_x,google_y,url,editorial,rights_managed,vote_like,vote_dislike,contacts,exclusive,url_jpg,url_png,url_gif,url_raw,url_tiff,url_eps,url_jpf,url_jp2,url_zip,content_type from " .
	...
	if ( ! $rs->eof ) {
		...
		$pvs_theme_content[ 'share_url' ] = urlencode( pvs_item_url( $rs->row["id"], $rs->row["url"] ) );
		...
		if ( ! $pvs_global_settings["zoomer"] or preg_match( "/icon_photo/", $preview_url ) or preg_match( "/adult_content/", $preview_url ) ) {
			if ( ! preg_match( "/icon_photo/", $preview_url ) )
			{
				$pvs_theme_content[ 'image' ] = "<center><img src='" . $preview_url . "' class='img-responsive'></center>";
			} else
			{
				$pvs_theme_content[ 'image' ] = "";
			}
		} else {
			if ( file_exists( $preview_url2 ) )
			{
			...
			$pvs_theme_content[ 'image' ] = "<center><iframe width=" . $iframe_width .
				" height=" . $iframe_height . " src='" . site_url() .
				"/content-photo-preview/?id=" . get_query_var('pvs_id') . "&width=" . $iframe_width .
				"&height=" . $iframe_height .
				"' frameborder='no' scrolling='no' class='d-none d-md-block hidden-xs hidden-sm hidden-phone hidden-md-down'></iframe><img src='" .
				$preview_url . "' class='img-responsive d-md-none hidden-md hidden-lg hidden-desktop hidden-tablet hidden-md-up nobootstrap'></center>";
		}
		...
		$pvs_theme_content[ 'flag_free' ] = $rs->row["free"];
		...
		//Show package buttons
		pvs_show_package( get_query_var('pvs_id'), 0 );
		...
		if ($pvs_global_settings["canvaspop"]) {			
			$canvaspop = "<div id='cp-store-root' data-cp-settings='{ \"access_key\": \"" . $pvs_global_settings["canvaspop_key"] . "\"}'></div>
			<script>
    (function ( d, s, id ) {
...
";
		...
		$pvs_theme_content[ 'canvaspop' ] = $canvaspop;
		...
		if ( $pvs_global_settings["prints"] ) {
			...
			$word_cart = pvs_word_lang( "add to cart" );
			if ( $pvs_global_settings[ 'no_cart' ] ) {
				$word_cart = pvs_word_lang( "Buy now" );
			}
			...
			if ( $rs->row["rights_managed"] == 0 )
			{
					...
					$folder ) )
				{
					...
					while ( ! $dd->eof )
					{
						...
						$sql = "select id_parent,title,size,jpg,png,gif,raw,tiff,eps,jp2,jpf,zip from " .
						...
						while ( ! $dr->eof )
						{
							$sql = "select id,name,url,price,contacts from " . PVS_DB_PREFIX .
								...
								if ( $ds->row["contacts"] == 1 )
									{
										...
										row["id"] . "'><td>" . pvs_word_lang( $ds->
										row["name"] ) . $subscription_link . "</td><td class='hidden-xs hidden-sm'>" . $formats . "</td><td class='hidden-xs hidden-sm'><div class='item_pixels'>" . $rw .
										"&nbsp;x&nbsp;" . $rh . "&nbsp;" . $size_photo .
										"</div><div class='item_inches' style='display:none'>" . $inches_string .
										"</div></td><td colspan='2' nowrap><a href='" .
										site_url() . "/contacts/?file_id=" . $rs->row["id"] . "&item=" . $ds->row["id"] . "'><u>" . pvs_word_lang( "contact us to get the price" ) . "</u></a></td></tr>";
									}
									else
									{
									$sizeboxes[$dd->row["id_parent"]] .= "<tr class='tr_cart' id='tr_cart" . $ds->
					...
					} else
					{
						...
						} else
						{
							...
							if ( $pvs_global_settings[ 'paypal_direct' ] ) {
								$sizebox .= "<a id='item_button_cart' href='javascript:paypal_payment()'><img src='" . pvs_plugins_url() . "/assets/images/paypal.png'></a>";
							} else {
								$sizebox .= "<input id='item_button_cart' class='add_to_cart' type='button' onclick=\"add_cart(0)\" value='" . $word_cart . "'>";
							}
							$sizebox .= "<h2 style='margin-top:20px'>" .
		...
		if ( file_exists ( get_stylesheet_directory(). '/item_photo.php' ) ) {
			require_once( get_stylesheet_directory(). '/item_photo.php' );
		} else {
			if ( file_exists ( PVS_PATH . 'templates/item_photo.php' ) ) {
				require_once( PVS_PATH . 'templates/item_photo.php' );
			}
		}
	}
}
