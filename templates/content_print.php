...
if ( pvs_check_password_publication( get_query_var('pvs_id') ) ) {
	...
	if ( ! $rs->eof ) {
		...
		if ( $default_height > $default_width) {
			$resize_min = round($resize_min*$default_width/$default_height);
			$resize_max = round($resize_max*$default_width/$default_height);
			$resize_value = round($resize_value*$default_width/$default_height);
		}
		...
		pvs_show_package( get_query_var('pvs_id'), ( int ) get_query_var('pvs_print_id') );
		...
		if ( file_exists ( get_stylesheet_directory(). '/item_print.php' ) ) {
			require_once( get_stylesheet_directory(). '/item_print.php' );
		} else {
			if ( file_exists ( PVS_PATH . 'templates/item_print.php' ) ) {
				require_once( PVS_PATH . 'templates/item_print.php' );
			}
		}
	}
}
