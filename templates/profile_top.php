...
?>
<div class="row" style="margin-top:20px"><div class="col-xl-3 col-lg-3 col-md-3 profile_left panel_left">
...
?></b></a>
	<span><a href="<?php echo (site_url( ) );?>/profile-about/"><?php echo pvs_word_lang( "edit" );?></a><br><a href="<?php echo wp_logout_url(site_url()); ?>"><?php echo pvs_word_lang( "logout" );?></a></span><br>
	<?php
	if ( pvs_is_user_admin () ) {
	?>
		<button onClick="location.href='<?php echo (site_url( ) );?>/wp-admin/'" class="btn btn-danger btn-xs" target="blank">WordPress Admin Panel</button>
	<?php
	}
	?>
	</div>
...
?>">
	...
	<?php
	if ( $sphoto == true and $pvs_global_settings["dropzone_uploader"] and $pvs_global_settings["allow_photo"] ) {
?>
		<li <?php
		if ( get_query_var('pvs_page') == "filemanager-photo-dropzone" ) {
			echo ( "class='activno'" );
		}
?>><a href="<?php echo (site_url( ) );?>/filemanager-photo-dropzone/"><?php echo pvs_word_lang( "Dropzone photo uploader" );?></a></li>
	<?php
	}
?>
...
<li id="icons_preview"><i class="glyphicon glyphicon-eye-open"> </i> <a href="<?php echo pvs_user_url( get_current_user_id() );?>"><?php echo pvs_word_lang( "preview" );?></a></li>
...
</div><div class="col-xl-9 col-lg-9 col-md-9 search_right panel_right">
	...
	{
		jQuery('#div_'+model_id.toString()).remove()
	...
	function change_group(value) 
	{
		...
		jQuery("li.menu_settings").removeClass('active');
		jQuery("li.menu_settings").removeClass('selected');
		jQuery("li.menu_settings_"+value).addClass('active');
		jQuery("li.menu_settings_"+value).addClass('selected');
		jQuery("li.menu_settings a").removeClass('active');
		jQuery("li.menu_settings_"+value+" a").addClass('active');
	...
	function translation_add(param_field,param_lang,param_lang2,param_lang3,param_type) {
		if(param_type=="input") {
			input_code="<input type='text' name='translate_"+param_field+"_"+param_lang3+"' class='ibox form-control' value=''>";
		}
		else {
			input_code="<textarea id='translate_"+param_field+"_"+param_lang3+"' name='translate_"+param_field+"_"+param_lang3+"' style='height:100px' class='ibox form-control'></textarea>";
		}
		
		jQuery('#trans_'+param_field).append("<div class='clearfix' id='div_"+param_field+"_"+param_lang3+"' style='padding-top:20px'><div class='input-append' style='float:left;margin-right:4px;width:80%'>"+input_code+"<span class='add-on' style='width:120px;text-align:left'><img src='<?php echo pvs_plugins_url();?>/assets/images/languages/"+param_lang2+".gif'>&nbsp;<font class='langtext'>"+param_lang+"</font></span></div><button class='btn btn-danger' type='button' onClick=\"translation_delete('"+param_field+"','"+param_lang3+"');\"><?php echo pvs_word_lang( "delete" )?></button></div>");
	...
	function translation_delete(param_field,param_lang3) {
		jQuery('#div_'+param_field+'_'+param_lang3).remove()
